		...
		>
			<div class="bg-[#0D1017]/90 backdrop-blur-2xl border border-white/10 rounded-[28px] p-5 flex flex-col gap-4 shadow-[0_12px_40px_rgba(0,0,0,0.5)]">
					...
					<div class="flex items-center gap-2.5">
						...
						<div class="flex flex-col">
							...
							<span class="text-[10px] font-bold text-white/40 uppercase tracking-wider">
								{assets()?.summaryText || (t('assets.subtitle' as any) || 'Reports, channels, projects & boosters')}
							</span>
						</div>
					</div>
				</div>
				...
				<div class="grid grid-cols-4 gap-1.5 p-1 bg-[#07090E] border border-white/5 rounded-[16px]">
					<For
						each={[
							{ id: 'reports', label: t('assets.reports' as any) || 'Reports', icon: 'description' },
							{ id: 'properties', label: t('assets.properties' as any) || 'Properties', icon: 'hub' },
							{ id: 'projects', label: t('assets.projects' as any) || 'Projects', icon: 'route' },
							{ id: 'boosters', label: t('assets.boosters' as any) || 'Boosters', icon: 'rocket_launch' },
						] as const}
					>
						{(tab) => (
							<button
								...
								<Show
									...
									fallback={
										<div class="py-8 text-center flex flex-col items-center gap-2">
											<span class="material-symbols-outlined text-[32px] text-white/20">search</span>
												...
												<div class="flex items-center gap-2">
													...
													>
														<span class="material-symbols-outlined text-[16px]">
															{reportNotifs()[r.username] !== false ? 'notifications_active' : 'notifications_off'}
														</span>
													</button>
						...
						<Show when={activeTab() === 'properties'}>
							<div class="flex flex-col gap-2">
								<Show
									...
									fallback={
										<div class="py-8 text-center flex flex-col items-center gap-2">
											<span class="material-symbols-outlined text-[32px] text-white/20">campaign</span>
											<span class="text-white/40 text-[11px] font-bold">
										...
										{(pj) => (
											<div class="p-3.5 bg-[#07090E] border border-white/5 rounded-[18px] flex flex-col gap-2.5">
												<div class="flex items-center justify-between">
													...
													<span class={`text-[9px] px-2 py-0.5 rounded-[8px] font-black uppercase ${
														pj.status === 'active'
												...
												<div class="flex items-center justify-between text-[10px] text-white/40 pt-0.5">
													<span>
														{pj.daysLeft > 0 ? `${pj.daysLeft} days remaining` : 'Subscription Expired'}
													</span>
													<button
							...
							<div class="flex flex-col gap-2.5">
								...
								<div class="p-3 bg-[#07090E] border border-white/5 rounded-[18px] flex items-center justify-between gap-3">
									<div class="flex items-center gap-3">
										...
										<div class="flex flex-col">
											<span class="text-[13px] font-black text-white">Energy Capacity</span>
											<span class="text-[10px] text-white/40">
												Level {assets()?.boosters.energyLimitLevel || 1} ({500 + ((assets()?.boosters.energyLimitLevel || 1) - 1) * 250} max)
											</span>
										</div>
									</div>
									<button
								...
								<div class="p-3 bg-[#07090E] border border-white/5 rounded-[18px] flex items-center justify-between gap-3">
									<div class="flex items-center gap-3">
										...
										<div class="flex flex-col">
											<span class="text-[13px] font-black text-white">Auto Tap-Bot</span>
											<span class="text-[10px] text-white/40">
												{assets()?.boosters.tapBotLevel ? 'Active (12h Offline Mining)' : 'Not Activated'}
											</span>
										</div>
									</div>
									<button
