				...
				<div class="flex items-end justify-between relative z-10 mb-4">
					<div class="flex flex-col gap-1.5">
						<div class="flex items-center gap-1.5">
							<span class="material-symbols-outlined text-[14px] text-[#00f5ff]" style="font-variation-settings: 'FILL' 1;">psychology</span>
							<h3 class="text-[12px] text-white/60 font-black uppercase tracking-widest">{t('profile.experience') || 'Experience'}</h3>
						</div>
						<div class="flex items-baseline gap-1">
							<span class="text-[32px] font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-white/70 leading-none tracking-tight">{(props.stats?.xp || 0).toLocaleString()}</span>
							<span class="text-[12px] font-black text-[#00f5ff]">{t('profile.xp') || 'XP'}</span>
						</div>
					</div>
					
					<div class="flex flex-col gap-1.5 text-right">
						<h3 class="text-[10px] text-white/40 font-bold uppercase tracking-widest">{t('profile.nextLevel') || 'Next Level'}</h3>
