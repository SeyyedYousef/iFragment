...
import { copyToClipboard, haptic, switchInlineQuery, shareToStory, showScanQrPopup } from '@/shared/lib/telegram-native.js';
import { initData } from '@tma.js/sdk-solid';
import type { ReferralInfo } from '@/shared/store/profile.js';

interface Props { referral: ReferralInfo | null }

export const ReferralPreview: Component<Props> = (props) => {
  const user = initData.user();
  const [copied, setCopied] = createSignal(false);

  const refLink = () => `https://t.me/iFragmentBot?start=${props.referral?.referralCode || `ref_${user?.id || '0'}`}`;
    ...
    <Motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
      ...
      <div class="flex items-center gap-2 mb-4">
        <div class="w-8 h-8 rounded-xl bg-[#0f1014] flex items-center justify-center border border-[#2a2a2a]">
          <span class="material-symbols-outlined text-[18px] text-[#34c759]" style={{ 'font-variation-settings': '"FILL" 1' }}>group_add</span>
        </div>
        <span class="text-white font-black text-sm">{t('profile.referralHub') || 'Referral Hub'}</span>
      </div>
      ...
      <div class="grid grid-cols-2 gap-2 mb-4">
        <div class="bg-[#0f1014] rounded-2xl p-3 border border-[#2a2a2a] text-center">
          ...
          <span class="text-[#a0a4ad] text-[10px] font-bold block mt-0.5">{t('profile.friendsInvited') || 'Friends Invited'}</span>
        </div>
        <div class="bg-[#0f1014] rounded-2xl p-3 border border-[#2a2a2a] text-center">
          <span class="text-[#34c759] font-black text-xl">{((props.referral?.totalEarned ?? 0) / 1000).toFixed(0)}K</span>
          <span class="text-[#a0a4ad] text-[10px] font-bold block mt-0.5">FRG {t('profile.earned') || 'Earned'}</span>
        </div>
      </div>
      ...
      <div class="bg-[#0f1014] rounded-2xl p-3 border border-[#2a2a2a] flex items-center gap-2 mb-3">
        ...
        <button onClick={handleCopy} class="px-3 py-1.5 rounded-xl bg-[#3390ec]/15 border border-[#3390ec]/30 text-[#3390ec] font-bold text-[11px] flex-shrink-0">
      ...
      <div class="grid grid-cols-3 gap-2">
        <button onClick={handleShare} class="py-2.5 rounded-2xl bg-[#3390ec]/10 border border-[#3390ec]/20 flex flex-col items-center gap-1 hover:bg-[#3390ec]/20 transition-colors">
        ...
        <button onClick={() => { haptic.impact('light'); shareToStory(refLink(), { text: 'Join me on iFragment!' }); }}
        ...
        <button onClick={handleScanQr} class="py-2.5 rounded-2xl bg-[#00c7e2]/10 border border-[#00c7e2]/20 flex flex-col items-center gap-1 hover:bg-[#00c7e2]/20 transition-colors">
