...
import { hapticFeedback, initData } from '@tma.js/sdk-solid';
...
export const IdentityHero: Component<Props> = (props) => {
	...
	return (
		<div
			class={`pt-10 pb-20 px-6 relative overflow-hidden border-b border-[#2a2a2a] ${skinClass() || 'bg-[#1c1c1c]'}`}
		...
		>
			...
			<div class="absolute top-4 end-4 z-20">
				...
				>
					...
					<span
						class="material-symbols-outlined text-[12px] transition-transform duration-200"
			...
			{/* Background glow */}
			<div
				class="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 rounded-full opacity-10 blur-3xl"
				style={{
					background:
						props.stats?.isPremium || user?.is_premium
							? 'radial-gradient(circle, #ffd700, transparent)'
							: 'radial-gradient(circle, #3390ec, transparent)',
				}}
			/>
			...
			>
				...
				<div class="relative mb-4">
					<div
						class={`w-28 h-28 rounded-full p-[3px] relative ${borderClass()}`}
						style={
							!borderClass()
								? {
										background:
					...
					<Show when={props.stats?.isPremium || user?.is_premium}>
						<div class="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-[#1c1c1c] flex items-center justify-center border-2 border-[#ffd700]">
							<span
								class="material-symbols-outlined text-[16px] text-amber-300"
				...
				<h1 class="text-white text-2xl font-black tracking-tight flex items-center gap-2 justify-center">
					...
					<Show when={props.stats?.isPremium || user?.is_premium}>
						<span
							class="material-symbols-outlined text-[18px] text-amber-300"
							style={{ 'font-variation-settings': '"FILL" 1' }}
						>
							verified
						</span>
				...
				<button
					onClick={() => handleCopy(`@${user?.username || 'guest'}`, 'username')}
					...
					{(info) => (
						...
						>
							<div class="flex items-center gap-2 bg-[#0f1014] px-4 py-1.5 rounded-full border border-[#2a2a2a]">
								<span class="text-[#3390ec] font-black text-xs">
									Lv.{formatNumber(info().current.level)}
								</span>
							...
							<div class="w-40 h-1.5 bg-[#2a2a2a] rounded-full overflow-hidden">
								<Motion.div
									...
									style={{ background: 'linear-gradient(90deg, #3390ec, #34c759)' }}
								/>
							</div>
							<span class="text-[10px] text-[#a0a4ad]">
								{formatNumber(props.stats?.xp ?? 0)} / {formatNumber(info().next.xpRequired)} XP
							</span>
						</Motion.div>
					)}
				</Show>
				...
				<div class="mt-4 flex gap-2 flex-wrap justify-center">
					<button
						...
						class="bg-[#0f1014] px-3 py-1.5 rounded-2xl border border-[#2a2a2a] flex items-center gap-1.5 hover:border-[#3a3a3a] transition-colors"
					>
						<span class="text-[10px] text-[#a0a4ad] uppercase font-bold tracking-widest">
							{t('profile.id')}
						</span>
						<span class="text-white font-mono font-bold text-xs">
							{user?.id ? formatNumber(user.id) : '---'}
						</span>
						<span class="material-symbols-outlined text-[12px] text-[#a0a4ad]">
					...
					<Show when={memberSince()}>
						<div class="bg-[#0f1014] px-3 py-1.5 rounded-2xl border border-[#2a2a2a] flex items-center gap-1.5">
							<span class="material-symbols-outlined text-[14px] text-[#34c759]">
								calendar_month
							</span>
					...
					<Show when={props.stats?.globalRank}>
						<div class="bg-[#0f1014] px-3 py-1.5 rounded-2xl border border-[#2a2a2a] flex items-center gap-1.5">
							<span
								class="material-symbols-outlined text-[14px] text-[#ffd700]"
								style={{ 'font-variation-settings': '"FILL" 1' }}
							>
								emoji_events
							</span>
							<span class="text-white font-bold text-xs">
								#{formatNumber(props.stats!.globalRank)}
							</span>
						</div>
					</Show>
				</div>
