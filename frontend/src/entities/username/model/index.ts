...
export const useUsernameSearch = () => {
  ...
  const validate = (val: string, tab?: 'username' | 'collectibles' | 'gifts') => {
    ...
    // Select correct schema based on tab context (use CollectibleUsernameSchema if length <= 4 since 4-char usernames must be collectibles)
    const schema = (tab === 'username' && cleanVal.length >= 5) ? StandardUsernameSchema : CollectibleUsernameSchema;
