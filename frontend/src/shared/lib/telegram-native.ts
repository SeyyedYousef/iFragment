...
export const haptic = {
  ...
  notification: (type: 'error' | 'success' | 'warning' = 'success') => {
    if (!profileSettings().hapticEnabled) return;
    try { getWebApp()?.HapticFeedback?.notificationOccurred(type); } catch {}
  },
  selection: () => {
    if (!profileSettings().hapticEnabled) return;
    try { getWebApp()?.HapticFeedback?.selectionChanged(); } catch {}
  },
};
...
// ─── Sharing ───
export const shareToStory = (mediaUrl: string, params?: { text?: string; widget_link?: { url: string; name?: string } }) => {
  try { getWebApp()?.shareToStory(mediaUrl, params); } catch {}
};

export const switchInlineQuery = (query: string, chatTypes?: string[]) => {
  try { getWebApp()?.switchInlineQuery(query, chatTypes); } catch {}
};

export const shareMessage = (msgId: string) => {
  try { getWebApp()?.shareMessage(msgId); } catch {}
};

// ─── Navigation ───
export const openTelegramLink = (url: string) => {
  try { getWebApp()?.openTelegramLink(url); } catch {}
};

export const openLink = (url: string, options?: { try_instant_view?: boolean }) => {
  try { getWebApp()?.openLink(url, options); } catch {}
};

// ─── Home Screen ───
export const addToHomeScreen = () => {
  try { getWebApp()?.addToHomeScreen(); } catch {}
};
...
export const setEmojiStatus = (customEmojiId: string, params?: { duration?: number }): Promise<boolean> => {
  return new Promise((resolve) => {
    try {
...
export const showScanQrPopup = (text?: string): Promise<string | null> => {
  return new Promise((resolve) => {
    try {
      getWebApp()?.showScanQrPopup(
        { text: text || 'Scan referral QR code' },
        (data: string) => {
...
export const biometric = {
  ...
  requestAccess: (reason: string): Promise<boolean> => {
    return new Promise((resolve) => {
      try {
        getWebApp()?.BiometricManager?.requestAccess({ reason }, (granted: boolean) => resolve(granted));
      } catch { resolve(false); }
    });
  },
  authenticate: (reason: string): Promise<boolean> => {
    return new Promise((resolve) => {
      try {
        getWebApp()?.BiometricManager?.authenticate({ reason }, (success: boolean) => resolve(success));
      } catch { resolve(false); }
    });
  },
};
...
export const downloadFile = (url: string, fileName: string): Promise<boolean> => {
  return new Promise((resolve) => {
    try {
      getWebApp()?.downloadFile({ url, file_name: fileName }, (accepted: boolean) => resolve(accepted));
    } catch { resolve(false); }
  });
};
...
export const getPlatform = (): string => {
  try { return getWebApp()?.platform || 'unknown'; } catch { return 'unknown'; }
};

export const getVersion = (): string => {
  try { return getWebApp()?.version || '0.0'; } catch { return '0.0'; }
};
