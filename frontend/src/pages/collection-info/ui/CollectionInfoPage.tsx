...
export const CollectionInfoPage: Component = () => {
	...
	const [activeVertical, setActiveVertical] = createSignal<'usernames' | 'numbers' | 'gifts'>(
		'usernames',
	);
	...
	// 2. Numbers Collection Query
	const numbersQuery = createQuery(() => ({
		queryKey: ['numbersIntelCollection'],
		queryFn: () => numbersApi.getIntel(),
		staleTime: 60 * 1000,
		enabled: activeVertical() === 'numbers',
	}));

	// 3. Gifts Collection Query
	const giftsQuery = createQuery(() => ({
		queryKey: ['giftsIntelCollection'],
		queryFn: () => giftsApi.getIntel(),
		staleTime: 45 * 1000,
		enabled: activeVertical() === 'gifts',
	}));
	...
	const openValuation = (handle: string) => {
		...
		if (activeVertical() === 'numbers') {
			navigate(`/numbers/report?n=${encodeURIComponent(handle)}`);
		} else if (activeVertical() === 'gifts') {
			navigate(`/gifts/report?g=${encodeURIComponent(handle)}`);
		} else {
	...
	return (
		...
		>
			...
			<div class="w-full max-w-[480px] mx-auto px-4 flex flex-col relative z-10 flex-1">
				...
				<div class="flex flex-col items-start pt-6 pb-2 px-1">
					...
					<h1 class="text-[28px] font-black tracking-tight text-white leading-none mb-2">
						{activeVertical() === 'usernames'
							...
							: activeVertical() === 'numbers'
								? t('numbers.intelTitle') || '+888 Numbers Collection'
								: t('gifts.intelTitle') || 'Telegram Gifts Market'}
					</h1>
					<p class="text-[12px] text-white/50 leading-relaxed font-medium">
						{activeVertical() === 'usernames'
							...
							: activeVertical() === 'numbers'
								? t('numbers.frozenDesc')
								: t('gifts.intelSubtitle')}
					</p>

					{/* 💎 PERSONAL USERNAME 1-TAP VALUATION HOOK */}
					<Show when={userHandle() && activeVertical() === 'usernames'}>
						<div
						...
						>
							<div class="flex items-center gap-3">
								...
								<div class="flex flex-col text-start">
									<span class="text-[9px] font-black uppercase tracking-widest text-[#0098EA]">
										{t('collectionInfo.yourTelegramAsset')}
									</span>
									<span
										class="text-[14px] font-mono font-bold text-white group-hover:text-[#0098EA] transition-colors"
										dir="ltr"
									>
										@{userHandle()}
									</span>
							...
							<div class="flex items-center gap-1.5 px-3 py-1.5 bg-[#0098EA] text-white rounded-[12px] text-[11px] font-black uppercase tracking-wider shadow-sm group-hover:shadow-[0_0_12px_rgba(0,152,234,0.5)] transition-all">
								<span>{t('collectionInfo.valuate')}</span>
								<span class="material-symbols-outlined text-[14px]">arrow_forward</span>
							</div>
						</div>
					</Show>

					{/* 🌐 3-WAY VERTICAL SWITCHER */}
					<div class="w-full grid grid-cols-3 gap-1.5 mt-4 p-1 bg-[#12141C]/90 border border-white/10 rounded-[18px]">
						<button
							type="button"
							onClick={() => {
								try {
							...
							class={`py-2 rounded-[14px] text-[11px] font-black uppercase tracking-wider transition-all ${
								activeVertical() === 'usernames'
									? 'bg-[#0098EA] text-white shadow-md'
									: 'text-white/40 hover:text-white/70'
							}`}
						>
							{t('collectionInfo.usernames')}
						</button>
						<button
							type="button"
							onClick={() => {
								...
								setActiveVertical('numbers');
							}}
							class={`py-2 rounded-[14px] text-[11px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 ${
								activeVertical() === 'numbers'
									? 'bg-[#0098EA] text-white shadow-md'
									: 'text-white/40 hover:text-white/70'
							}`}
						>
							<span>{t('collectionInfo.numbers')}</span>
							<span class="text-[8px] bg-white/20 px-1 py-0.5 rounded text-white font-bold">
								+888
							</span>
						</button>
						<button
						...
						>
							<span>{t('collectionInfo.gifts')}</span>
							<span class="text-[8px] bg-white/20 px-1 py-0.5 rounded text-white font-bold">
								{'6 MKTS'}
							</span>
						</button>
					...
					<form onSubmit={handleSearchSubmit} class="w-full mt-3">
						<div class="relative w-full flex items-center">
							<input
								...
								placeholder={
									activeVertical() === 'numbers'
										? '+888 8888 8888 or 1234 5678'
										: activeVertical() === 'gifts'
											? 'Plush Pepe or gift name...'
											: t('collectionInfo.searchPlaceholder')
								}
					...
					{/* NAVIGATION TABS */}
					<div class="w-full bg-[#12141C]/90 border border-white/10 rounded-[18px] p-1.5 flex gap-1.5 mt-3 shadow-inner backdrop-blur-xl">
						<button
							type="button"
							onClick={() => {
								try {
				...
				{/* ═══════ STATUS SCREENS ═══════ */}
				<Show when={usernameQuery.isLoading || numbersQuery.isLoading || giftsQuery.isLoading}>
				...
				<Show when={activeTab() === 'overview' && !usernameQuery.isLoading}>
					<div class="flex flex-col gap-4 mt-3">
						...
						{/* ═══════ VERTICAL 1: USERNAMES OVERVIEW ═══════ */}
						<Show when={activeVertical() === 'usernames'}>
						...
						{/* ═══════ VERTICAL 2: NUMBERS OVERVIEW ═══════ */}
						<Show when={activeVertical() === 'numbers'}>
							<div class="grid grid-cols-2 gap-3">
								<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[20px] p-4 flex flex-col justify-between">
									<span class="text-[10px] text-white/40 uppercase tracking-widest font-black block mb-1">
										{t('collectionInfo.floorPrice')}
									</span>
									<div class="flex items-baseline gap-1" dir="ltr">
										<span class="text-[22px] font-black font-mono text-white">
											{formatTon(numbersQuery.data?.floor_price_ton || 2100)}
										</span>
										<span class="text-[11px] text-[#0098EA] font-black">{t('common.ton')}</span>
									</div>
									<div class="text-[10px] text-white/40 font-mono mt-1">
										≈ {formatUsd(numbersQuery.data?.floor_price_usd || 11550)}
									</div>
								</div>

								<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[20px] p-4 flex flex-col justify-between">
									<span class="text-[10px] text-white/40 uppercase tracking-widest font-black block mb-1">
										{t('collectionInfo.totalSupply')}
									</span>
									<div class="flex items-baseline gap-1" dir="ltr">
										<span class="text-[22px] font-black font-mono text-emerald-400">136,566</span>
									</div>
									<div class="text-[10px] text-emerald-400 font-mono mt-1">
										{t('collectionInfo.frozenCollection')}
									</div>
								</div>
						...
						{/* ═══════ VERTICAL 3: GIFTS OVERVIEW ═══════ */}
						<Show when={activeVertical() === 'gifts'}>
							<div class="grid grid-cols-2 gap-3">
								<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[20px] p-4 flex flex-col justify-between">
									<span class="text-[10px] text-white/40 uppercase tracking-widest font-black block mb-1">
										{t('collectionInfo.aggregatedMarkets')}
									</span>
									<div class="flex items-baseline gap-1" dir="ltr">
								...
								<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[20px] p-4 flex flex-col justify-between">
									<span class="text-[10px] text-white/40 uppercase tracking-widest font-black block mb-1">
										{t('collectionInfo.cumulativeVol')}
									</span>
									<div class="flex items-baseline gap-1" dir="ltr">
										<span class="text-[22px] font-black font-mono text-white">
											{formatUsd(giftsQuery.data?.total_cumulative_volume_usd || 292450000)}
										</span>
									</div>
									<div class="text-[10px] text-emerald-400 font-mono mt-1">
										{t('collectionInfo.dropstabVerified')}
									</div>
								</div>

								<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[20px] p-4 flex flex-col justify-between">
									<span class="text-[10px] text-white/40 uppercase tracking-widest font-black block mb-1">
										{t('collectionInfo.activeWallets')}
									</span>
									<div class="flex items-baseline gap-1" dir="ltr">
										<span class="text-[22px] font-black font-mono text-white">540K+</span>
									</div>
									<div class="text-[10px] text-white/40 font-mono mt-1">
										{t('collectionInfo.holdersOnTon')}
									</div>
								</div>

								<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[20px] p-4 flex flex-col justify-between">
									<span class="text-[10px] text-white/40 uppercase tracking-widest font-black block mb-1">
										{t('collectionInfo.craftingForge')}
									</span>
									<div class="flex items-baseline gap-1" dir="ltr">
										<span class="text-[22px] font-black font-mono text-amber-400">10K</span>
										<span class="text-[11px] text-amber-400 font-black">{'RUNS'}</span>
									</div>
									<div class="text-[10px] text-amber-400 font-mono mt-1">
										{t('collectionInfo.monteCarloEvSim')}
									</div>
								</div>
							</div>
				...
				<Show when={activeTab() === 'leaderboard'}>
					<div class="flex flex-col gap-4 mt-3">
							...
							<div class="flex items-center gap-3 mb-2">
								...
								<div class="flex flex-col text-start">
									<h2 class="text-[16px] font-black text-white tracking-tight">
										{activeVertical() === 'numbers'
											? t('collectionInfo.allTimeRecordNumbers')
											: activeVertical() === 'gifts'
												? t('collectionInfo.highestValuedGifts')
						...
						<div class="bg-[#12141C]/80 backdrop-blur-2xl border border-white/10 rounded-[24px] overflow-hidden shadow-sm">
							...
							<Show when={activeVertical() === 'numbers'}>
								<For each={numbersQuery.data?.hall_of_fame || []}>
									{(item) => (
										<div
											onClick={() => openValuation(item.number)}
											class="flex items-center justify-between p-4 hover:bg-white/[0.04] transition-all cursor-pointer border-b border-white/5 last:border-0 active:scale-[0.99]"
										>
											<div class="flex items-center gap-3.5 min-w-0">
												<div class="w-8 h-8 rounded-[10px] bg-amber-400/15 border border-amber-400/30 flex items-center justify-center font-black font-mono text-[12px] text-amber-400 shrink-0">
													#{item.rank}
												</div>
												<div class="flex flex-col min-w-0 text-start">
							...
							{/* Gifts Top Models */}
							<Show when={activeVertical() === 'gifts'}>
								<For each={giftsQuery.data?.unified_floor_board || []}>
									{(item, idx) => (
										<div
											onClick={() => openValuation(`${item.model_id}-1`)}
											class="flex items-center justify-between p-4 hover:bg-white/[0.04] transition-all cursor-pointer border-b border-white/5 last:border-0 active:scale-[0.99]"
										>
											<div class="flex items-center gap-3.5 min-w-0">
												<div class="w-8 h-8 rounded-[10px] bg-[#0098EA]/15 border border-[#0098EA]/30 flex items-center justify-center font-black font-mono text-[12px] text-[#0098EA] shrink-0">
													#{idx() + 1}
												</div>
												<div class="flex flex-col min-w-0 text-start">
													<span class="text-white font-black text-[14px] truncate">
														{item.name}
													</span>
