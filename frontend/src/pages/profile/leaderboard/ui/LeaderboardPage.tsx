		...
		<div class="min-h-screen bg-[#0f1014] pb-32 text-white font-sans">
			...
			<div class="relative bg-gradient-to-b from-[#1a1b23] to-[#0f1014] pt-12 pb-8 px-6 text-center border-b border-[#222]">
				...
				<h1 class="text-2xl font-black tracking-tight text-white mb-1">
					{t('gamification.globalLeaderboard') || 'Global Leaderboard'}
				</h1>
				<p class="text-xs text-[#a0a4ad]">
					{t('gamification.leaderboardSubtitle') || 'Top 100 elite iFragment miners worldwide'}
				</p>
				...
				<div class="flex justify-center items-end gap-4 mt-8">
					...
					<Show when={leaderboard().length >= 2}>
						<div class="flex flex-col items-center">
							<div class="w-12 h-12 rounded-full bg-[#c0c0c0]/10 border-2 border-[#c0c0c0] flex items-center justify-center text-lg font-black shadow-lg">
								🥈
							</div>
							<span class="text-[10px] font-bold mt-2 max-w-[70px] truncate">
								{leaderboard()[1].first_name}
							</span>
							<span class="text-[9px] text-[#c0c0c0]">
					...
					<Show when={leaderboard().length >= 1}>
						<div class="flex flex-col items-center -translate-y-2">
							<div class="w-16 h-16 rounded-full bg-[#ffd700]/10 border-2 border-[#ffd700] flex items-center justify-center text-2xl font-black shadow-[0_0_15px_rgba(255,215,0,0.2)]">
								🥇
							</div>
							<span class="text-xs font-black mt-2 max-w-[85px] truncate text-[#ffd700]">
								{leaderboard()[0].first_name}
							</span>
							<span class="text-[10px] text-[#ffd700] font-black">
					...
					<Show when={leaderboard().length >= 3}>
						<div class="flex flex-col items-center">
							<div class="w-12 h-12 rounded-full bg-[#cd7f32]/10 border-2 border-[#cd7f32] flex items-center justify-center text-lg font-black shadow-lg">
								🥉
							</div>
							<span class="text-[10px] font-bold mt-2 max-w-[70px] truncate">
								{leaderboard()[2].first_name}
							</span>
							<span class="text-[9px] text-[#cd7f32]">
						...
						{(member) => (
							<div class="flex items-center justify-between bg-[#15161d]/60 border border-[#222]/80 rounded-2xl p-4 transition-all hover:bg-[#1c1d26]">
								<div class="flex items-center gap-3">
									{/* Rank Badge */}
									<div
										class={`w-9 h-9 rounded-xl border flex items-center justify-center text-xs font-black ${getRankColor(member.rank)}`}
									>
									...
									<div class="flex flex-col">
										...
										<span class="text-[10px] text-[#a0a4ad] font-semibold">
											{t('gamification.levelLabel')?.replace('{level}', member.level.toString()) ||
												`Level ${member.level}`}
										</span>
									</div>
								</div>
								...
								<div class="flex flex-col items-end">
									<span class="text-xs font-black text-[#3390ec]">
										{member.xp.toLocaleString()}
									</span>
									<span class="text-[9px] text-[#a0a4ad] font-bold uppercase tracking-widest">
										XP
									</span>
								</div>
							</div>
					...
					<div class="flex items-center gap-3">
						...
						<div class="flex flex-col">
							<span class="text-xs font-black text-white">
								{t('gamification.yourGlobalRank') || 'Your Global Rank'}
							</span>
							<span class="text-[10px] text-[#a0a4ad] font-semibold">
