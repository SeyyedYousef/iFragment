...
import { Component, createSignal, ErrorBoundary, onMount, Show } from 'solid-js';
import { useSecretTrigger } from '@/features/owner-gate/lib/useSecretTrigger.js';
import { getProfileAchievements, getProfileStats, getReferralInfo } from '@/shared/api/profile.js';
import { isRtl, t } from '@/shared/i18n/index.js';
import { addToHomeScreen, checkHomeScreenStatus } from '@/shared/lib/telegram-native.js';
...
export const ProfilePage: Component = () => {
	...
	return (
		<div class="min-h-screen bg-[#0f1014] pb-32 text-white">
				...
				<ErrorBoundary fallback={(err, reset) => <ErrorFallback err={err} reset={reset} />}>
					<div class="flex flex-col">
						...
						>
							...
							<button class="absolute top-9 end-10 w-8 h-8 rounded-full bg-[#0f1014]/40 border border-[#2a2a2a] flex items-center justify-center group-active:scale-95 transition-all">
								<span
									class={`material-symbols-outlined text-[16px] text-white transition-transform ${isRtl() ? 'rotate-180' : ''}`}
								>
									chevron_right
								</span>
							</button>
						...
						>
							...
							>
								...
								<span
									class={`material-symbols-outlined text-[#a0a4ad] text-[18px] transition-transform ${isRtl() ? 'rotate-180 group-hover:-translate-x-0.5' : 'group-hover:translate-x-0.5'}`}
								>
									chevron_right
								</span>
							</button>
							...
							>
								<div class="flex items-center gap-3">
									<div class="w-9 h-9 rounded-xl bg-[#0f1014] flex items-center justify-center border border-[#2a2a2a]">
										<span class="material-symbols-outlined text-[20px] text-[#34c759]">
											security
										</span>
									</div>
									<span class="text-xs font-black text-white">
										{t('security.title') || 'Security & Keys'}
									</span>
								</div>
								<span
									class={`material-symbols-outlined text-[#a0a4ad] text-[18px] transition-transform ${isRtl() ? 'rotate-180 group-hover:-translate-x-0.5' : 'group-hover:translate-x-0.5'}`}
						...
						<div class="mt-8 mb-6 text-center flex flex-col items-center gap-1 opacity-40">
							...
							>
								...
								{t('profile.tmaProduction') || 'TMA Production'})
							</span>
						</div>
					</div>
				</ErrorBoundary>
			...
			<RedeemPromoModal isOpen={showPromoModal()} onClose={() => setShowPromoModal(false)} />
		</div>
	);
};
