...
const BOOSTER_META: Record<
	string,
	{ nameKey: string; descKey: string; icon: string; color: string }
> = {
	tapPower: {
		nameKey: 'airdrop.boosters.tapPower',
		descKey: 'airdrop.boosters.tapPowerDesc',
		icon: 'touch_app',
		color: '#fbbf24',
	},
	energyCap: {
		nameKey: 'airdrop.boosters.energyCap',
		descKey: 'airdrop.boosters.energyCapDesc',
		icon: 'bolt',
		color: '#3390ec',
	},
	tapBot: {
		nameKey: 'gamification.tapBotDesc',
		descKey: 'gamification.tapBotDesc',
		icon: 'smart_toy',
		color: '#34c759',
	},
};

export const BoostersView: Component<{ hideHeader?: boolean }> = (props) => {
	...
	return (
		<div class="flex-1 overflow-y-auto px-4 pt-4 pb-8 animate-fade-in no-scrollbar">
			<Show when={!props.hideHeader}>
				<SectionHeader
					icon="rocket_launch"
					title={t('airdrop.boosters.title')}
					subtitle={t('airdrop.boosters.subtitle')}
			...
			<div class="space-y-3 mt-4">
				<For each={Object.keys(boosters())}>
					{(id) => {
						...
						return (
							<div
								class={`bg-[#1c1c1e]/80 backdrop-blur-lg rounded-2xl p-4 border transition-all ${animatingId() === id ? 'scale-[1.02]' : 'border-white/[0.04]'}`}
								style={
									animatingId() === id
										? { 'border-color': meta.color, 'box-shadow': `0 0 15px ${meta.color}30` }
										: {}
								}
							>
									...
									<div class="flex-1 min-w-0">
										<div class="flex items-center gap-2 mb-0.5">
											<span class="text-white font-bold text-sm">
												{t(meta.nameKey as import('@/shared/i18n/index.js').DictPaths)}
											</span>
											<span
											...
											>
												{isMaxed()
													? t('airdrop.boosters.maxed')
													: `${t('airdrop.boosters.level')}${booster().level}`}
											</span>
										</div>
										<p class="text-[11px] text-[#8e8e93] font-medium">
											{t(meta.descKey as import('@/shared/i18n/index.js').DictPaths)}
										</p>
