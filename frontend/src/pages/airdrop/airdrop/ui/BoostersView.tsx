		...
		>
			...
			<div class="flex flex-col items-center mb-7 text-center relative z-10 pt-2">
				<div class="w-20 h-20 rounded-[24px] bg-gradient-to-br from-[#1c1608] to-[#08090D] border-[1.5px] border-amber-500/30 flex items-center justify-center mb-4 shadow-[inset_0_2px_12px_rgba(255,255,255,0.05),0_10px_30px_rgba(245,158,11,0.15)] relative overflow-hidden">
					...
					<span class="material-symbols-outlined text-[42px] text-amber-400 drop-shadow-[0_0_12px_rgba(245,158,11,0.6)]">rocket_launch</span>
				</div>
			...
			<div class="mb-7 relative z-10">
				...
				<div class="grid grid-cols-2 gap-3">
					{/* Turbo Button */}
					<button
						onClick={() => { spawnRocket(); props.onTurboClick?.(); }}
					...
					>
						...
						<div class="flex items-center justify-between w-full mb-1 z-10">
							<span class="text-white font-black text-[14px] tracking-tight">{t('airdropFinal.boosters.turbo')}</span>
					...
					{/* Full Energy Button */}
					<button
						onClick={() => { activateFullEnergy(); props.onTurboClick?.(); }}
					...
					>
						...
						<div class="flex items-center justify-between w-full mb-1 z-10">
							<span class="text-white font-black text-[14px] tracking-tight">{t('airdropFinal.boosters.fullEnergy')}</span>
							<span class="text-[22px] drop-shadow-md">⚡</span>
						</div>
						<span class="text-cyan-400 font-mono font-bold text-[12px] bg-cyan-400/10 px-2 py-0.5 rounded-[6px] border border-cyan-400/20 z-10">
							{t('airdropFinal.boosters.available' as any).replace('{count}', `${fullEnergyCount()}/3`)}
						</span>
					</button>
				</div>
			</div>
				...
				<div class="bg-[#12141C]/80 backdrop-blur-xl rounded-[28px] p-2 flex flex-col border border-white/5 shadow-[0_10px_40px_rgba(0,0,0,0.3)]">
					<For each={Object.keys(boosters())}>
						{(id) => {
							...
							const iconMap: Record<string, string> = { tapPower: 'touch_app', energyCap: 'battery_charging_full', tapBot: 'smart_toy' };
							...
							return (
								<button
									...
									class={`flex items-center p-3.5 my-0.5 transition-all duration-300 text-start rounded-[20px] border 
										${isMaxed() || !canAfford() ? 'border-transparent opacity-50 cursor-not-allowed grayscale-[0.4]' : 'bg-[#161b28]/40 border-white/5 hover:border-white/15 hover:bg-[#1a2133] active:bg-white/10'} 
								...
								>
									...
									<div class={`w-12 h-12 rounded-[16px] flex items-center justify-center shrink-0 border shadow-inner transition-colors
										${isBotActive ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-[#08090D] border-white/10 text-white/80'}`
									}>
										<span class="material-symbols-outlined text-[24px]">{iconMap[id] || 'bolt'}</span>
									</div>
									...
									<div class="flex-1 flex flex-col items-start min-w-0 justify-center px-3.5">
										<div class="flex items-center gap-2.5 mb-1 flex-wrap">
											<span class="text-white font-black text-[15px] tracking-tight">
												{metaName !== `airdropFinal.boosters.meta.${id}.name` ? metaName : defaultMeta.name}
											</span>
											{isBotActive && (
												<span class="text-emerald-400 text-[9px] bg-emerald-500/10 border border-emerald-500/30 px-1.5 py-0.5 rounded-[6px] flex items-center gap-1 font-black uppercase tracking-widest shadow-sm">
													ACTIVE <div class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_5px_#34d399]" />
												</span>
											)}
										</div>
										...
										{isBotActive ? (
											<span class="text-white/50 text-[11px] font-medium leading-tight">
												{metaDesc !== `airdropFinal.boosters.meta.${id}.desc` ? metaDesc : defaultMeta.desc}
											</span>
										) : isMaxed() ? (
											...
											<div class="flex items-center gap-1.5 opacity-90">
												...
												<span class="text-white font-mono font-black text-[13px]">{cost().toLocaleString('en-US')}</span>
									...
									<div class="shrink-0 pl-2">
										{isMaxed() && id !== 'tapBot' ? (
											...
											<span class="material-symbols-outlined text-emerald-400 text-[24px] drop-shadow-sm">check_circle</span>
										) : (
											<span class="material-symbols-outlined text-white/30 text-[24px] rtl:rotate-180 group-hover:text-white/60 transition-colors">chevron_right</span>
										)}
									</div>
								</button>
							);
