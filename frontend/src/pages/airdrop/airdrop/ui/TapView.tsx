		...
		<div class="theme-play flex-1 flex flex-col items-center relative overflow-hidden bg-[#08090D] text-white select-none">
			...
			<div
				...
				style={{
					background: isTurboActive()
						? 'radial-gradient(circle at 50% 50%, rgba(239,68,68,0.5) 0%, rgba(245,158,11,0.2) 40%, transparent 70%)'
						: 'radial-gradient(circle at 50% 50%, rgba(245,158,11,0.3) 0%, rgba(255,255,255,0.05) 45%, transparent 70%)',
					filter: 'blur(50px)',
				}}
			/>
			...
			<Show when={isRocketSpawned()}>
				<button
					...
					class="absolute z-[70] text-[56px] drop-shadow-[0_0_20px_rgba(245,158,11,0.8)]"
			...
			<div class="w-full px-4 mt-4 relative z-20" dir="rtl">
				<button
					...
					class="w-full bg-[#151822] border border-white/10 rounded-[20px] flex items-center justify-between p-3.5 active:scale-95 transition-all shadow-lg"
				>
					<Show
						when={userClan()}
						fallback={
							...
							<div class="flex items-center justify-between w-full">
								<div class="flex items-center gap-3">
									<div class="w-10 h-10 rounded-xl bg-black border border-white/10 flex items-center justify-center overflow-hidden">
										<Show when={clan().channel_photo} fallback={<span class="text-xs font-bold text-amber-400">🛡️</span>}>
									...
									<div class="flex flex-col text-start">
										...
										<span class="text-[10px] font-mono text-amber-400">{(clan().total_score || 0).toLocaleString()} امتیاز</span>
									</div>
								</div>
			...
			<button
				...
				dir="ltr"
			>
				<div class="w-10 h-10 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 shadow-lg shadow-amber-500/30 flex items-center justify-center shrink-0 border border-amber-300">
					<span class="text-black text-xl font-black">¢</span>
				</div>
				<span class="text-white font-black text-5xl tracking-tight font-mono drop-shadow-md">
			...
			<div class="flex-1 flex flex-col items-center justify-center w-full relative z-10 py-4">
				<div class={`relative flex items-center justify-center w-[80vw] max-w-[320px] aspect-square ${isShaking() ? 'animate-shake' : ''}`}>
					<button
						...
						class="relative w-[280px] h-[280px] rounded-full z-20 flex items-center justify-center touch-none select-none bg-[#0F1117] border-4 border-amber-500/30 shadow-2xl shadow-amber-500/10 active:scale-95 transition-transform"
					>
						<Show when={showCombo()}>
							<div class="absolute top-6 right-6 text-amber-400 font-black text-2xl font-mono animate-bounce">
								x{comboCount()}
							</div>
						</Show>

						<svg viewBox="0 0 100 100" class="w-[50%] h-[50%] text-amber-400 fill-current drop-shadow-lg">
			...
			<div class="w-full flex items-center justify-between px-6 mb-24 relative z-30" dir="rtl">
				...
				<div class="flex items-center bg-[#151822] border border-white/10 rounded-2xl p-1 gap-1">
					<button
						onClick={() => props.onActionClick?.('frens')}
						class="px-3 py-1.5 rounded-xl text-xs font-bold text-white/70 hover:text-white flex items-center gap-1"
					>
