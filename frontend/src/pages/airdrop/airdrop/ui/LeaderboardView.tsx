      ...
      >
        <div class="bg-[#3390ec]/10 border border-[#3390ec]/20 rounded-2xl p-4 mb-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="px-2 min-w-[36px] h-9 rounded-full bg-[#3390ec]/20 flex items-center justify-center text-[#3390ec] font-black text-sm">#{userPosition() !== '?' ? Number(userPosition()).toLocaleString('en-US') : '?'}</div>
            <div>
        ...
        >
          ...
          >
            <For each={leaderboardQuery.data}>
              {(entry, i) => (
                <div class={`flex items-center justify-between px-4 py-3.5 ${i() < (leaderboardQuery.data?.length || 0) - 1 ? 'border-b border-white/[0.04]' : ''}`}>
                  <div class="flex items-center gap-3">
