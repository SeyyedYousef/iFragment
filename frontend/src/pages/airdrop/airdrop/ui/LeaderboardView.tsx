			...
			<div class="relative z-10 flex flex-col gap-5 pt-4 max-w-md mx-auto">
				...
				<Show when={activeTab() === 'miners'}>
					<div class="mx-4 mt-2 rounded-[28px] border border-white/10 bg-gradient-to-b from-[#12141C] to-[#08090D] p-6 flex flex-col items-center relative overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.6)]">
						{/* Dynamic Core Glow */}
						<div
							class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full blur-3xl opacity-10 pointer-events-none transition-all duration-700"
						...
						<div class="flex items-center justify-between w-full relative z-10 mb-4">
							<button
								...
								class={`w-10 h-10 rounded-[14px] flex items-center justify-center transition-all ${selectedLeagueIndex() === 0 ? 'opacity-20' : 'bg-white/5 hover:bg-white/10 active:scale-95 border border-white/10'}`}
							>
							...
							<div class="flex flex-col items-center">
								<div
									class="w-16 h-16 rounded-[20px] flex items-center justify-center mb-2 shadow-inner border border-white/5"
									style={{
										background: `linear-gradient(135deg, ${currentLeague().color}25, rgba(255,255,255,0.02))`,
									}}
								>
									<span
										class="material-symbols-outlined text-[36px] drop-shadow-lg transition-colors duration-500"
										style={{ color: currentLeague().color, 'font-variation-settings': '"FILL" 1' }}
									>
										{currentLeague().icon}
									</span>
								</div>
								<h2 class="text-white font-black text-[16px] uppercase tracking-[0.2em] drop-shadow-sm">
									{currentLeague().name}
								</h2>
							</div>
							...
							<button
								...
								class={`w-10 h-10 rounded-[14px] flex items-center justify-center transition-all ${selectedLeagueIndex() === LEAGUES.length - 1 ? 'opacity-20' : 'bg-white/5 hover:bg-white/10 active:scale-95 border border-white/10'}`}
							>
						...
						<div class="w-full max-w-[260px] relative z-10 mt-2">
							<div
								class="flex items-center justify-between text-[11px] font-mono font-bold mb-2 text-white/50"
								dir="ltr"
							>
								<span class="text-white">
									{(statsQuery.data?.xp || 0).toLocaleString('en-US')} XP
								</span>
								<span>
									{formatScore(
							...
							<div class="w-full h-[6px] bg-black/80 rounded-full overflow-hidden border border-white/10 shadow-inner">
								<div
									...
									style={{
										...
										background: `linear-gradient(90deg, ${currentLeague().color}88, ${currentLeague().color})`,
										'box-shadow': `0 0 10px ${currentLeague().color}80`,
									}}
								>
									<div
									...
									{(clan) => {
										...
										return (
											<div class="flex flex-col items-center w-[31%] h-[160px] bg-gradient-to-t from-[#12141C] to-[#1a202c] border border-slate-400/30 rounded-t-[24px] rounded-b-[16px] p-2 relative shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
												...
												<span class="text-white font-bold text-[12px] truncate w-full text-center mb-0.5 tracking-tight px-1">
													{clan().chat_title}
												</span>
												<span class="text-slate-300 font-black text-[12px] mb-auto tabular-nums mt-1">
													🪙 {formatScore(score)}
												</span>
												<button
													type="button"
													onClick={() => openChannel(clan().channel_username)}
													class="w-full py-1.5 rounded-[10px] bg-white/5 hover:bg-slate-300 text-slate-300 hover:text-black text-[10px] font-black tracking-wider transition-all border border-slate-300/20 active:scale-95"
												>
													{t('leaderboard.join')}
												</button>
											</div>
										...
										return (
											<div class="flex flex-col items-center w-[36%] h-[190px] bg-gradient-to-t from-[#12141C] to-[#2d220b] border-[1.5px] border-amber-400/60 rounded-t-[28px] rounded-b-[20px] p-2.5 relative shadow-[0_0_40px_rgba(245,158,11,0.25)] z-10">
												...
												<div class="relative w-16 h-16 mb-3 mt-[-36px]">
													...
													<div
														class="absolute -top-6 left-1/2 -translate-x-1/2 text-[26px] drop-shadow-[0_0_10px_rgba(251,191,36,0.8)] z-20"
												...
												<span class="text-white font-black text-[14px] truncate w-full text-center mb-0.5 tracking-tight px-1">
													{clan().chat_title}
												</span>
												<span class="text-white/40 text-[9px] font-mono uppercase tracking-widest">
													{formatScore(clan().members_count)} Mem
												</span>
												<span class="text-amber-400 font-black text-[14px] mb-auto tabular-nums mt-1 drop-shadow-md">
													🪙 {formatScore(score)}
												</span>
												<button
													type="button"
													onClick={() => openChannel(clan().channel_username)}
													class="w-full py-2 rounded-[12px] bg-gradient-to-r from-amber-400 to-amber-500 text-black text-[11px] font-black tracking-wider transition-all shadow-[0_4px_12px_rgba(245,158,11,0.3)] active:scale-95"
												>
													{t('leaderboard.join')}
												</button>
											</div>
									...
									{(clan) => {
										...
										return (
											<div class="flex flex-col items-center w-[31%] h-[150px] bg-gradient-to-t from-[#12141C] to-[#261811] border border-orange-500/30 rounded-t-[24px] rounded-b-[16px] p-2 relative shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
												...
												<span class="text-white font-bold text-[11px] truncate w-full text-center mb-0.5 tracking-tight px-1">
													{clan().chat_title}
												</span>
												<span class="text-orange-400 font-black text-[11px] mb-auto tabular-nums mt-1">
													🪙 {formatScore(score)}
												</span>
												<button
													type="button"
													onClick={() => openChannel(clan().channel_username)}
													class="w-full py-1.5 rounded-[10px] bg-white/5 hover:bg-orange-500 text-orange-400 hover:text-black text-[10px] font-black tracking-wider transition-all border border-orange-500/20 active:scale-95"
												>
													{t('leaderboard.join')}
												</button>
											</div>
					...
					<Show when={activeTab() === 'miners'}>
						...
						>
							<For
								each={filteredMiners()}
								fallback={
									<div class="text-center py-10 text-white/40 text-[13px]">
										{t('airdropFinal.leaderboard.noMiners', { defaultValue: 'No miners found.' })}
									</div>
								}
							>
								{(entry, i) => {
									...
									return (
										<div
											class={`flex items-center justify-between p-3.5 rounded-[20px] bg-[#12141C]/80 backdrop-blur-xl border border-white/5 hover:border-white/15 transition-all shadow-sm ${isTop ? 'bg-gradient-to-r from-white/5 to-transparent' : ''}`}
										>
											<div class="flex items-center gap-3.5 min-w-0 pr-2">
												...
												<div class="flex flex-col min-w-0">
													<span class="text-white font-bold text-[14px] truncate">
														{entry.name}
													</span>
													<Show when={entry.clanName}>
														{(cn) => (
															<span
																class="text-[#3390ec] text-[11px] font-mono font-bold truncate mt-0.5"
																dir="ltr"
															>
																@{cn().replace(/^@+/, '')}
															</span>
											...
											<div
												class="shrink-0 pl-2 font-mono font-black text-[13px] text-white/90 tabular-nums flex items-center gap-1"
												dir="ltr"
											>
											...
											<div class="flex items-center justify-between p-3.5 rounded-[20px] bg-[#12141C]/80 backdrop-blur-xl border border-white/5 hover:border-white/15 transition-all group shadow-sm">
												<div class="flex items-center gap-3.5 min-w-0 pr-2">
													...
													<div class="flex flex-col min-w-0">
														...
														<span class="text-[#3390ec] text-[11px] font-mono mt-0.5" dir="ltr">
															@{cleanTelegramUsername(clan.channel_username)}
														</span>
													</div>
												</div>
												<div class="flex items-center gap-3 shrink-0 pl-2">
													<div
														class="font-mono font-black text-[13px] text-white/90 tabular-nums flex items-center gap-1"
														dir="ltr"
													>
													...
													<button
														type="button"
														onClick={() => openChannel(clan.channel_username)}
														class="w-9 h-9 rounded-[12px] bg-white/5 hover:bg-[#3390ec] text-white/60 hover:text-white border border-white/10 flex items-center justify-center shrink-0 active:scale-95 transition-all"
													>
										...
										<div class="flex items-center justify-between p-4 rounded-[24px] border-[1.5px] border-amber-400/50 bg-gradient-to-r from-amber-400/10 to-[#12141C] shadow-[0_8px_32px_rgba(245,158,11,0.15)] relative overflow-hidden">
											<div class="flex items-center gap-3.5 min-w-0 pr-2 z-10">
												...
												<div class="flex flex-col min-w-0">
													...
													<span class="text-amber-300 text-[11px] font-mono mt-0.5" dir="ltr">
														@{cleanTelegramUsername(info().clan.channel_username)}
													</span>
												</div>
											</div>
											<div class="flex items-center gap-3 shrink-0 pl-2 z-10">
												<div
													class="font-mono font-black text-[14px] text-amber-400 tabular-nums flex items-center gap-1 drop-shadow-md"
													dir="ltr"
												>
													<span class="text-[15px]">🪙</span>{' '}
													{formatScore(info().clan.total_score || info().clan.members_count * 1500)}
												</div>
												<button
													type="button"
													onClick={() => openChannel(info().clan.channel_username)}
													class="w-10 h-10 rounded-[14px] bg-amber-400 text-black flex items-center justify-center shrink-0 active:scale-95 transition-all shadow-md"
												>
