...
export const MarketplacePage: Component = () => {
  ...
  const handleConvert = async (e: Event) => {
    ...
    if (isNaN(coinsVal) || coinsVal < 100000) {
  ...
  return (
    ...
    >
      ...
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[350px] h-[350px] rounded-full pointer-events-none filter blur-[80px]" style={{ background: 'radial-gradient(circle, rgba(51,144,236,0.15) 0%, transparent 70%)' }}></div>

      {/* Header Info */}
      <div class="px-5 pt-6 pb-4 z-10 flex flex-col items-center">
        <span class="material-symbols-outlined text-amber-400 text-5xl mb-2 animate-bounce" style={{ 'font-variation-settings': '"FILL" 1' }}>storefront</span>
        <h1 class="text-white text-2xl font-black tracking-tight">{t('marketplace.title') || 'بازارچه سکه'}</h1>
        <p class="text-[#8e8e93] text-xs font-semibold mt-1 text-center">{t('marketplace.subtitle') || 'خرید و تبدیل سکه FRG'}</p>
      </div>
      ...
      <div class="px-4 mb-6 z-10">
        <div class="bg-gradient-to-br from-[#1c1c1e] to-[#121214] border border-white/[0.05] rounded-3xl p-5 shadow-2xl relative overflow-hidden flex items-center justify-between">
          ...
          <div>
            <div class="text-[#8e8e93] text-[10px] font-black uppercase tracking-widest">{t('marketplace.balance') || 'موجودی سکه (FRG)'}</div>
          ...
          <div class="w-14 h-14 rounded-2xl bg-amber-400/5 border border-amber-400/20 flex items-center justify-center shadow-inner">
            <span class="material-symbols-outlined text-amber-400 text-3xl" style={{ 'font-variation-settings': '"FILL" 1' }}>toll</span>
          </div>
        </div>
      </div>
        ...
        <Show when={activeTab() === 'buy'}>
          ...
          >
            <div class="grid grid-cols-2 gap-3.5">
              <For each={options()}>
                {(opt) => (
                  <div class={`bg-[#1c1c1e]/80 backdrop-blur-lg border rounded-3xl p-4 flex flex-col justify-between relative overflow-hidden transition-all duration-300 ${
                    opt.popular 
                    ...
                    <div>
                      ...
                      <div class="text-white text-2xl font-black flex items-baseline gap-0.5 font-mono mt-1">
                        {opt.frg_amount}
                        <span class="text-amber-400 text-[10px] font-black font-sans ml-0.5">FRG</span>
                      </div>
        ...
        <Show when={activeTab() === 'convert'}>
          <div class="bg-[#1c1c1e]/80 backdrop-blur-lg border border-white/[0.04] rounded-3xl p-5">
            ...
            <form onSubmit={handleConvert} class="flex flex-col gap-5">
              <div>
                <label class="block text-[#8e8e93] text-xs font-bold mb-2.5 px-1">سکه جهت تبدیل:</label>
                <div class="relative">
                  <input
                    type="number"
                    value={convertAmount()}
                  ...
                  <div class="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center gap-1 pointer-events-none">
                    <span class="material-symbols-outlined text-amber-400 text-lg" style={{ 'font-variation-settings': '"FILL" 1' }}>monetization_on</span>
                ...
                <div class="flex gap-2 mt-3">
                  <button type="button" onClick={() => setPercentAmount(0.25)} class="flex-1 bg-[#2c2c2e]/40 hover:bg-[#2c2c2e]/80 border border-white/[0.02] text-[#8e8e93] hover:text-white font-bold py-2 rounded-xl text-[10px] transition-colors">25%</button>
                  <button type="button" onClick={() => setPercentAmount(0.50)} class="flex-1 bg-[#2c2c2e]/40 hover:bg-[#2c2c2e]/80 border border-white/[0.02] text-[#8e8e93] hover:text-white font-bold py-2 rounded-xl text-[10px] transition-colors">50%</button>
                  <button type="button" onClick={() => setPercentAmount(0.75)} class="flex-1 bg-[#2c2c2e]/40 hover:bg-[#2c2c2e]/80 border border-white/[0.02] text-[#8e8e93] hover:text-white font-bold py-2 rounded-xl text-[10px] transition-colors">75%</button>
                  <button type="button" onClick={() => setPercentAmount(1.0)} class="flex-1 bg-[#2c2c2e]/40 hover:bg-[#2c2c2e]/80 border border-white/[0.02] text-[#8e8e93] hover:text-white font-bold py-2 rounded-xl text-[10px] transition-colors">MAX</button>
                </div>
              </div>
              ...
              <Show when={calculatedFRG() > 0}>
                <div class="bg-[#3390ec]/5 border border-[#3390ec]/20 rounded-2xl p-4 flex items-center justify-between animate-fade-in">
                  <div>
                    <div class="text-[#8e8e93] text-[9px] font-black uppercase tracking-wider">سکه دریافتی:</div>
