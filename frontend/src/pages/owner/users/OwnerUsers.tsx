...
export const OwnerUsers: Component = () => {
  ...
  const handleSearch = async (e?: Event) => {
    ...
    try {
      const resp = await apiClient.get(`/owner/users/search?q=${encodeURIComponent(query().trim())}`);
      setUsers(resp.data || []);
      ...
      if (resp.data.success) {
        ...
        setUsers(users().map(u => 
          u.telegram_id === user.telegram_id 
            ? { ...u, balance: resp.data.new_balance } 
            : u
        ));
      ...
      <div class="px-6 pt-6 pb-4 flex items-center justify-between border-b border-white/5 relative z-10">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[9px] text-[#3390ec] font-black uppercase tracking-widest mt-0.5">Manager</p>
          </div>
        </div>
      </div>
        ...
        <div class="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-[#000000]/80 backdrop-blur-md animate-fade-in">
          <div class="w-full max-w-sm bg-gradient-to-b from-[#1c1d22] to-[#121316] border border-[#2a2c35]/50 rounded-[32px] p-6 shadow-2xl relative">
            ...
            <form onSubmit={submitFrgAdjustment} class="flex flex-col gap-4">
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[9px] text-[#a0a4ad] font-black uppercase tracking-wider">Audit Adjustment Reason</label>
                <textarea 
      ...
      <Show when={showBanModal() && selectedUser()}>
        <div class="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-[#000000]/80 backdrop-blur-md animate-fade-in">
          <div class="w-full max-w-sm bg-gradient-to-b from-[#1c1d22] to-[#121316] border border-[#2a2c35]/50 rounded-[32px] p-6 shadow-2xl relative">
            ...
            <h2 class="text-sm font-black uppercase tracking-wider text-white mb-4">Account Suspension / Lock</h2>
            <p class="text-[10px] text-[#a0a4ad] font-bold mb-4">
              Restricts user actions across iFragment platform. Safe security log tracking applies automatically.
            </p>

            <form onSubmit={submitBanOperation} class="flex flex-col gap-4">
              <div class="flex flex-col gap-1.5">
                <label class="text-[9px] text-[#a0a4ad] font-black uppercase tracking-wider">Lock Severity Level</label>
                <select 
                  value={banType()}
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[9px] text-[#a0a4ad] font-black uppercase tracking-wider">Lock Duration</label>
                <select 
                  value={banDuration()}
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[9px] text-[#a0a4ad] font-black uppercase tracking-wider">Reason for Suspension</label>
                <textarea 
