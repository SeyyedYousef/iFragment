                  ...
                  <div class="bg-[#121318]/90 border border-white/5 rounded-2xl p-5 shadow-inner hover:border-white/10 transition-all">
                    <div class="flex justify-between items-start gap-2">
                      <div>
                        <div class="flex items-center gap-2 flex-wrap">
                          ...
                          <span class={`px-2 py-0.5 text-[8px] font-black uppercase rounded-md select-none ${
                            !q.is_active || isExpired 
                              ? 'bg-red-500/10 text-red-400 border border-red-500/10' 
                              : 'bg-green-500/10 text-green-400 border border-green-500/10'
                          }`}>
                            {!q.is_active ? t('owner.quests.inactive') : isExpired ? t('owner.quests.expired') : t('owner.quests.active')}
                          </span>
                        </div>
                        <p class="text-[10px] text-white/50 font-bold font-mono mt-1 select-all">{q.key}</p>
                      </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 text-xs font-bold">
                      <div class="flex flex-col gap-1">
                        <span class="text-[9px] uppercase tracking-wider text-white/40">{t('owner.quests.rewardFrg')}</span>
                        <span class="text-white text-sm font-black flex items-center gap-1">💰 {q.reward_frg.toLocaleString()}</span>
                      </div>
                      <div class="flex flex-col gap-1">
                        <span class="text-[9px] uppercase tracking-wider text-white/40">{t('owner.quests.rewardXp')}</span>
                        <span class="text-white text-sm font-black flex items-center gap-1">⚡ {q.reward_xp.toLocaleString()}</span>
                      </div>
                    </div>

                    <div class="mt-4 text-[10px] font-bold text-white/30 flex justify-between items-center">
                      <span>
                        {t('owner.quests.expiry')}: <span class="text-white/60">{q.expires_at ? new Date(q.expires_at).toLocaleDateString() : t('owner.quests.noExpiry')}</span>
                      </span>
                    </div>
                  </div>
                );
          ...
          <div class="w-full max-w-md bg-gradient-to-b from-[#1c1d22] to-[#121316] border border-[#2a2c35]/50 rounded-[32px] p-6 shadow-2xl relative my-8">
            ...
            <form onSubmit={handleSubmit} class="space-y-4">
              <div class="flex flex-col gap-1.5">
                <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.key')}</label>
                <input
                  type="text"
                  required
                  disabled={isEditing()}
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.questTitle')}</label>
                <input
                  type="text"
                  required
                  value={title()}
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                  <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.rewardFrg')}</label>
                  <input
                    type="number"
                    min="0"
                    required
                ...
                <div class="flex flex-col gap-1.5">
                  <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.rewardXp')}</label>
                  <input
                    type="number"
                    min="0"
                    required
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.type')}</label>
                <select
                  value={type()}
              ...
              <Show when={type() === 'channel_join'}>
                <div class="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-3 animate-fade-in">
                  <div class="flex flex-col gap-1.5">
                    <label class="text-[9px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.channelUsername')}</label>
                    <input
                      type="text"
                      required
              ...
              <Show when={type() === 'quiz'}>
                <div class="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-3 animate-fade-in">
                  <div class="flex flex-col gap-1.5">
                    <label class="text-[9px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.quizQuestion')}</label>
                    <input
                      type="text"
                      required
                      value={quizQuestion()}
                  ...
                  <div class="flex flex-col gap-1.5">
                    <label class="text-[9px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.quizAnswer')}</label>
                    <input
                      type="text"
                      required={!isEditing()}
                      value={quizAnswer()}
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                  <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.expiry')}</label>
                  <input
                    type="date"
                    value={expiresAt()}
                ...
                <div class="flex flex-col gap-1.5">
                  <label class="text-[10px] font-black uppercase text-[#a0a4ad]">{t('owner.quests.status')}</label>
                  <button
                    type="button"
