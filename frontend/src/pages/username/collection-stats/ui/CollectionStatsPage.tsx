        ...
        <Show when={stats.data}>
          ...
          <div class="bg-[#141518] border border-[#2a2a2a] p-5 rounded-[24px]">
            ...
            <div class="h-2 w-full bg-[#1c1c1c] rounded-full overflow-hidden flex mb-4">
              <div class="h-full bg-[#3390ec]" style={{ width: `${Number(distribution()?.single || 0)}%` }} title="1 Item" />
              <div class="h-full bg-blue-600" style={{ width: `${Number(distribution()?.small || 0)}%` }} title="2-5 Items" />
              <div class="h-full bg-cyan-500" style={{ width: `${Number(distribution()?.medium || 0)}%` }} title="6-24 Items" />
              <div class="h-full bg-teal-500" style={{ width: `${Number(distribution()?.large || 0)}%` }} title="25-50 Items" />
              <div class="h-full bg-orange-500" style={{ width: `${Number(distribution()?.whale || 0)}%` }} title="50+ Items" />
            </div>
...
const DistributionItem: Component<{ label: string; color: string; value: number | string; wide?: boolean }> = (props) => (
