					...
					<div class="w-full max-w-[420px] flex flex-col items-center gap-4 relative z-10">
								...
								<div class="w-full max-w-[440px] flex flex-col items-center gap-4 relative z-10 pt-2 pb-12">
									...
									<div class="w-full bg-[#12141C]/90 backdrop-blur-2xl border border-white/10 rounded-[28px] p-5 flex flex-col gap-3.5 shadow-2xl relative overflow-hidden text-start">
										...
										<div class="relative flex items-center w-full">
											...
											<input
												...
												onInput={(e) =>
													setSearchTerm(
														e.currentTarget.value.replace(/^@/, '').toLowerCase().trim(),
													)
												}
												onKeyDown={(e) => {
										...
										<div class="grid grid-cols-2 gap-2.5 pt-1">
											...
											>
												<span
													class={`material-symbols-outlined text-[18px] text-cyan-400 ${verifyingOnChain() ? 'animate-spin' : ''}`}
												>
										...
										{(ver) => (
											<div class="w-full bg-[#12141C]/95 backdrop-blur-2xl border border-emerald-500/30 rounded-[24px] p-4 flex flex-col gap-3 shadow-xl text-start">
												<div class="flex items-center justify-between border-b border-white/5 pb-2.5">
													...
													<span
														class={`text-[9px] font-mono font-black uppercase px-2 py-0.5 rounded-[6px] border ${
															ver().is_minted_nft && ver().collection_verified
												...
												<div class="flex flex-col gap-1.5 text-[11px] font-mono text-white/70">
													...
													<Show when={ver().telemint_provenance?.item_address}>
														<div class="flex justify-between items-center">
															...
															>
																{ver().telemint_provenance.item_address!.slice(0, 6)}...
																{ver().telemint_provenance.item_address!.slice(-4)} ↗
															</a>
														</div>
													</Show>
												</div>
													...
													{(rep) => (
														...
														>
															<div class="flex flex-col min-w-0">
																<span
																	class="text-white font-mono font-bold text-[13px] truncate"
																	dir="ltr"
																>
																	@{rep.username}
																</span>
															...
															<div class="flex flex-col items-end shrink-0">
																<span class="text-emerald-400 font-mono font-black text-[12px]">
																	{rep.expectedTon
																		? `${fmtTon(parseFloat(rep.expectedTon))} TON`
																		: 'View'}
																</span>
									...
									<div class="w-full grid grid-cols-2 gap-2.5 pt-2">
										<div class="p-3.5 rounded-[20px] bg-[#12141C]/80 border border-white/5 flex flex-col gap-1 text-start">
											...
											<span class="text-[10px] font-mono text-white/50" dir="ltr">
												@news (Fragment)
											</span>
										</div>
										<div class="p-3.5 rounded-[20px] bg-[#12141C]/80 border border-white/5 flex flex-col gap-1 text-start">
						...
						>
											...
											<Show when={verificationData()}>
												{(ver) => (
													<div class="flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-mono text-white/70 z-10 my-1">
														<span
															class={`w-2 h-2 rounded-full ${ver().is_minted_nft ? 'bg-emerald-400' : 'bg-amber-400'}`}
														/>
														<span class="font-bold">
															{ver().is_minted_nft ? 'TELEMINT NFT (ON-CHAIN)' : 'UNMINTED HANDLE'}
														</span>
							...
							<Show when={accessGranted() && data()}>
								...
								<div class="w-full bg-[#12141C]/90 backdrop-blur-2xl border border-white/10 rounded-[28px] p-5 shadow-xl text-start flex flex-col gap-3.5 relative overflow-hidden">
									...
									<div class="flex items-center justify-between border-b border-white/5 pb-3 relative z-10">
										<div class="flex items-center gap-2.5 min-w-0">
											<div
												class={`w-9 h-9 rounded-[12px] flex items-center justify-center shrink-0 border ${
													(data()?.telemint_provenance?.is_authentic ??
													verificationData()?.telemint_provenance?.is_authentic)
														? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400'
														: (
																	data()?.telemint_provenance?.collection_match === false ||
																		verificationData()?.collection_verified === false
																)
											...
											<div class="flex flex-col min-w-0">
												<h4 class="text-[13px] font-black text-white uppercase tracking-wider truncate">
													{isRtl()
														? 'اصالت قرارداد هوشمند آن‌چین (Telemint)'
														: 'ON-CHAIN TELEMINT AUTHENTICITY'}
												</h4>
												<span class="text-[10px] text-white/40 font-mono truncate">
													{isRtl()
														? 'تایید مستقیم قرارداد بر بستر شبکه TON'
														: 'TEP-62 Smart Contract Verification'}
												</span>
											</div>
										</div>
										<span
											class={`text-[9px] font-mono font-black uppercase px-2.5 py-1 rounded-[8px] border shrink-0 ${
												(data()?.telemint_provenance?.is_authentic ??
												verificationData()?.telemint_provenance?.is_authentic)
									...
									<div class="flex flex-col gap-2 relative z-10 text-[11px] font-mono">
										...
										<Show
											when={
												data()?.telemint_provenance?.item_address ||
												verificationData()?.telemint_provenance?.item_address
											}
										>
											{(addr) => (
										...
										<Show
											when={
												data()?.telemint_provenance?.is_escrow ||
												verificationData()?.telemint_provenance?.is_escrow
											}
										>
								...
								<div class="w-full grid grid-cols-2 gap-2.5">
									...
									<div class="p-3.5 rounded-[20px] bg-[#12141C]/90 backdrop-blur-xl border border-white/10 flex flex-col justify-between text-start shadow-sm">
										<div class="flex items-center justify-between gap-1 mb-1.5">
											...
											<span class="material-symbols-outlined text-xs text-[#0098EA]">
												storefront
											</span>
										</div>
										<div class="flex items-center gap-1.5">
								...
								<Show
									when={
										(data()?.homoglyph_twins && data()!.homoglyph_twins!.length > 0) ||
										data()?.risk_audit?.has_homoglyph_risk
									}
								>
										...
										<div class="flex items-center justify-between border-b border-white/5 pb-3">
											<div class="flex items-center gap-2.5 min-w-0">
												...
												<div class="flex flex-col min-w-0">
													<h4 class="text-[13px] font-black text-white uppercase tracking-wider truncate">
														{isRtl()
															? 'دوقلوهای بصری و ردیابی فیشینگ'
															: 'CONFUSABLE TWINS & SPOOF RADAR'}
													</h4>
													<span class="text-[10px] text-white/40 font-mono truncate">
														{isRtl()
															? 'کاراکترهای مشابه‌نما (Homoglyph substitutions)'
															: 'Visual confusable substitution attack vectors'}
													</span>
												</div>
											</div>
													...
													{(twin) => (
														<div class="bg-[#08090D] border border-white/5 rounded-[16px] p-3 flex items-center justify-between gap-2">
															<div class="flex flex-col min-w-0">
																<div class="flex items-center gap-2">
																	<span
																		class="text-white font-mono font-black text-[13px]"
																		dir="ltr"
																	>
																		@{twin.twin}
																	</span>
																	<span
																		class={`text-[9px] font-mono font-black uppercase px-2 py-0.5 rounded-[5px] border ${
									...
									<div class="grid grid-cols-2 gap-2.5">
										...
										<div class="col-span-2 bg-gradient-to-r from-emerald-950/30 to-[#08090D] border border-emerald-500/20 rounded-[18px] p-3.5 flex items-center justify-between">
											<div class="flex flex-col">
												...
												<span class="text-[10px] font-mono text-white/40">
													≈ ${fmtUsd(Math.max(0, parseFloat(data()?.expected_usd || '0') * 0.95))}{' '}
													USD
												</span>
											</div>
								...
								<div class="w-full bg-[#12141C]/90 backdrop-blur-2xl border border-white/10 rounded-[28px] p-5 shadow-xl text-start">
									...
									<div class="grid grid-cols-3 gap-2 text-center">
										...
										<div class="p-3 rounded-2xl bg-white/[0.03] border border-white/5 min-w-0">
											...
											>
												{fmtTon(
													data()?.empirical_band?.model_low_ton ||
														data()?.empirical_band?.p10_ton ||
														Math.round(expectedTon() * 0.7),
												)}{' '}
												<span class="text-[9px] text-[#0098EA]">TON</span>
											</span>
											<span
												class="text-[9px] text-white/40 font-mono block mt-0.5 truncate"
												dir="ltr"
											>
												≈ $
												{fmtUsd(
													data()?.empirical_band?.model_low_usd ||
														data()?.empirical_band?.p10_usd ||
														Math.round(
															(data()?.empirical_band?.p10_ton || expectedTon() * 0.7) *
																(data()?.ton_usd_rate || 0),
														),
												)}
											</span>
										...
										<div class="p-3 rounded-2xl bg-[#0098EA]/10 border border-[#0098EA]/30 min-w-0 shadow-sm">
											...
											>
												{fmtTon(
													data()?.empirical_band?.model_mid_ton ||
														data()?.empirical_band?.p50_ton ||
														Math.round(expectedTon()),
												)}{' '}
												<span class="text-[9px] text-[#0098EA]">TON</span>
											</span>
											<span
											...
											>
												≈ $
												{fmtUsd(
													data()?.empirical_band?.model_mid_usd ||
														data()?.empirical_band?.p50_usd ||
														Math.round(
															(data()?.empirical_band?.p50_ton || expectedTon()) *
																(data()?.ton_usd_rate || 0),
														),
												)}
											</span>
										...
										<div class="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 min-w-0">
											...
											>
												{fmtTon(
													data()?.empirical_band?.model_high_ton ||
														data()?.empirical_band?.p90_ton ||
														Math.round(expectedTon() * 1.35),
												)}{' '}
												<span class="text-[9px] text-[#0098EA]">TON</span>
											</span>
											<span
											...
											>
												≈ $
												{fmtUsd(
													data()?.empirical_band?.model_high_usd ||
														data()?.empirical_band?.p90_usd ||
														Math.round(
															(data()?.empirical_band?.p90_ton || expectedTon() * 1.35) *
																(data()?.ton_usd_rate || 0),
														),
												)}
											</span>
								...
								>
									<div class="flex items-center justify-between border-b border-white/5 pb-3">
										...
										>
											{data()?.certificate_id ||
												(data()?.run_id
													? `IFRG-USR-${data()!.run_id.toString(16).toUpperCase()}`
													: 'IFRG-USR-CERT')}
										</span>
									</div>
											...
											<div class="bg-[#08090D] border border-white/5 rounded-[14px] p-3 flex items-center justify-between gap-2 text-start">
												<div class="flex flex-col min-w-0">
													<span class="text-[9px] font-black text-white/40 uppercase tracking-wider flex items-center gap-1">
														<span class="material-symbols-outlined text-[13px] text-amber-400">
															fingerprint
														</span>
								...
								<div class="w-full bg-[#12141C]/80 backdrop-blur-2xl border border-white/5 rounded-[28px] p-6 flex flex-col gap-4 shadow-sm">
									...
									<div class="grid grid-cols-2 gap-3">
										...
										<div class="bg-[#08090D] border border-white/5 rounded-[18px] p-3.5 flex flex-col gap-1">
											...
											<span class="text-white/30 text-[9px]">
												{t('valuation.maxBidCompetition')}
											</span>
										</div>
									...
									<Show when={data()?.history?.owner_address}>
										<div class="flex items-center justify-between bg-[#08090D] border border-white/5 rounded-[18px] p-3.5 text-start gap-2">
											<div class="flex flex-col min-w-0">
												...
												<span
													class="text-white font-mono font-black text-[12px] truncate"
													dir="ltr"
												>
										...
										<div class="flex items-center justify-between text-white/90 relative z-10 border-b border-white/5 pb-3">
											<div class="flex items-center gap-2.5 min-w-0">
												...
												<div class="flex flex-col text-start min-w-0">
													...
													<span class="text-[10px] text-white/40 font-medium truncate">
														{t('valuation.whale_portfolio_subtitle') ||
															'ON-CHAIN ASSET DISTRIBUTION'}
													</span>
												</div>
											</div>
												...
												each={(() => {
													...
													if (f === 'word') {
														const filtered = list.filter(
															(i) =>
																(i.reason || '').toLowerCase().includes('word') ||
																(i.reason || '').toLowerCase().includes('dictionary') ||
																(i.reason || '').toLowerCase().includes('semantic') ||
																(i.reason || '').toLowerCase().includes('brand'),
														);
													...
													if (f === 'structure') {
														const filtered = list.filter(
															(i) =>
																(i.reason || '').toLowerCase().includes('char') ||
																(i.reason || '').toLowerCase().includes('length') ||
																(i.reason || '').toLowerCase().includes('structure') ||
																(i.reason || '').toLowerCase().includes('pattern'),
														);
														return filtered.length > 0 ? filtered : list;
													}
													if (f === 'price') {
								...
								<div class="w-full flex gap-3 mt-2">
									...
									>
										...
										<span>
											{sharing() ? 'Preparing...' : t('valuation.share') || 'Share Story'}
										</span>
									</button>
									<button
										type="button"
