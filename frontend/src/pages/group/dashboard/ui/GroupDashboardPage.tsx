...
export const GroupDashboardPage: Component = () => {
	...
	const groupFeatures = () => [
		{
			name: t('search.features.groupSettings'),
			icon: 'settings',
			path: `/group/${params.id}/settings`,
		},
		{
			name: t('search.features.contentRestrictions'),
			icon: 'block',
			path: `/group/${params.id}/content`,
		},
		...
		{
			name: t('search.features.mandatoryChannels'),
			icon: 'how_to_reg',
			path: `/group/${params.id}/mandatory`,
		},
		{
			name: t('search.features.customTexts'),
			icon: 'edit_note',
		...
		{ name: t('groupDashboard.membersAndWarnings'), icon: 'group', path: `/group/${params.id}/members` },
		{
			...
			icon: 'analytics',
			path: `/group/${params.id}/analytics`,
		},
		{
			name: t('search.features.groupDynamicBio'),
			icon: 'badge',
			path: `/group/${params.id}/dynamic-bio`,
		},
	];
	...
	const confirmToggleGroupLock = async () => {
		...
		try {
			...
			showToast(
				current ? t('groupDashboard.unlockSuccess') : t('groupDashboard.lockSuccess'),
				'success',
			);
	...
	return (
		<div
			class="min-h-screen bg-[#030303] pb-28 relative overflow-x-hidden text-white select-none font-sans selection:bg-[#3390ec]/30"
			dir={isRtl() ? 'rtl' : 'ltr'}
		>
			{/* Ambient Top Glow */}
				...
				<div class="flex items-center gap-3 overflow-hidden flex-1">
					...
					<div class="flex flex-col overflow-hidden">
						...
						<div class="flex items-center gap-1.5 text-[10px] text-white/50 font-bold mt-0.5 tracking-wider uppercase">
							...
							<span
								class={
									group()?.subscription_status === 'paid' ? 'text-[#10b981]' : 'text-amber-400'
								}
							>
								{group()?.subscription_status === 'paid'
									? t('groupDashboard.proBadge')
									: t('groupDashboard.freeBadge')}
							</span>
							<span class="w-1 h-1 rounded-full bg-white/20" />
							<span class="font-mono">
								{t('groupDashboard.membersCount', { count: group()?.members_count || 0 })}
							</span>
						</div>
					</div>
				</div>
			...
			<div class="px-5 pt-6 flex flex-col gap-5 max-w-md mx-auto relative z-10">
					...
					<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[28px] p-5 flex flex-col gap-3.5 shadow-sm">
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-2 text-[#3390ec]">
								...
								<h3 class="text-[13px] font-black uppercase tracking-widest">
									{t('groupDashboard.telegramSecurityStatus')}
								</h3>
							</div>
						...
						<div class="grid grid-cols-2 gap-2 text-[11px]">
							<div
								class={`p-2.5 rounded-[14px] border flex items-center gap-2 ${tgInfo()?.has_protected_content ? 'bg-[#10b981]/10 border-[#10b981]/30 text-[#10b981]' : 'bg-white/5 border-white/5 text-white/40'}`}
							>
								<span class="material-symbols-outlined text-[16px]">lock</span>
								<span class="font-bold">{t('groupDashboard.protectedContent')}</span>
							</div>
							<div
								class={`p-2.5 rounded-[14px] border flex items-center gap-2 ${tgInfo()?.has_hidden_members ? 'bg-[#10b981]/10 border-[#10b981]/30 text-[#10b981]' : 'bg-white/5 border-white/5 text-white/40'}`}
							>
								<span class="material-symbols-outlined text-[16px]">visibility_off</span>
								<span class="font-bold">{t('groupDashboard.hideMembers')}</span>
							</div>
							<div
								class={`p-2.5 rounded-[14px] border flex items-center gap-2 ${tgInfo()?.has_aggressive_anti_spam_enabled ? 'bg-[#10b981]/10 border-[#10b981]/30 text-[#10b981]' : 'bg-white/5 border-white/5 text-white/40'}`}
							>
								<span class="material-symbols-outlined text-[16px]">shield</span>
								<span class="font-bold">{t('groupDashboard.nativeAntispam')}</span>
							</div>
							<div
								class={`p-2.5 rounded-[14px] border flex items-center gap-2 ${tgInfo()?.join_by_request ? 'bg-[#10b981]/10 border-[#10b981]/30 text-[#10b981]' : 'bg-white/5 border-white/5 text-white/40'}`}
							>
								<span class="material-symbols-outlined text-[16px]">verified_user</span>
								<span class="font-bold">{t('groupDashboard.joinByRequest')}</span>
							</div>
						</div>
					</div>
				</Show>
				...
				<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[28px] p-5 flex flex-col gap-4 shadow-sm relative overflow-hidden">
					...
					>
						...
						<span>
							{isGroupLocked()
								? t('groupDashboard.emergencyLockActive')
								: t('groupDashboard.quickLockGroup')}
						</span>
					</button>
				</div>
				...
				<div class="bg-gradient-to-r from-[#10b981]/15 to-[#3390ec]/10 border border-[#10b981]/30 rounded-[24px] p-4 flex flex-col gap-2 shadow-sm relative overflow-hidden">
					<div class="flex items-center gap-3">
						<div class="w-10 h-10 rounded-[12px] bg-[#10b981]/20 border border-[#10b981]/40 flex items-center justify-center shrink-0">
							<span class="material-symbols-outlined text-[#10b981] text-[22px]">
								verified_user
							</span>
						</div>
						<div class="flex flex-col">
							...
							<p class="text-[10px] text-white/60 font-medium mt-0.5">
								{t('groupCommands.adFreeNotice' as any) ||
									'100% Ad-Free & Privacy First. No promotional ads or airdrop spam will ever be sent to your group.'}
							</p>
						</div>
					</div>
				</div>
				...
				<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[24px] p-4 flex flex-col gap-3">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-2">
							...
							<span class="text-[13px] font-black text-white">
								{t('groupCommands.title' as any) || 'Fast Admin Chat Commands'}
							</span>
						</div>
						<span class="text-[10px] font-mono text-white/40 uppercase font-bold">
							{t('groupCommands.inChatCli' as any) || 'In-Chat CLI'}
						</span>
					</div>
					<div class="grid grid-cols-2 gap-2 text-[11px] font-mono">
						<div class="bg-black/30 rounded-[12px] p-2 border border-white/5 flex flex-col">
							...
							<span class="text-[10px] text-white/40 font-sans">
								{t('groupCommands.settingsDesc' as any) || 'Interactive Inline GUI'}
							</span>
						</div>
						<div class="bg-black/30 rounded-[12px] p-2 border border-white/5 flex flex-col">
							<span class="text-[#3390ec] font-bold">
								{t('groupCommands.lockUnlock' as any) || '/lock & /unlock'}
							</span>
							<span class="text-[10px] text-white/40 font-sans">
								{t('groupCommands.lockDesc' as any) || 'Lock/Open chat'}
							</span>
						</div>
						<div class="bg-black/30 rounded-[12px] p-2 border border-white/5 flex flex-col">
							<span class="text-[#3390ec] font-bold">
								{t('groupCommands.muteDur' as any) || '/mute [10m|1h]'}
							</span>
							<span class="text-[10px] text-white/40 font-sans">
								{t('groupCommands.muteDesc' as any) || 'Mute with duration'}
							</span>
						</div>
						<div class="bg-black/30 rounded-[12px] p-2 border border-white/5 flex flex-col">
							<span class="text-[#3390ec] font-bold">
								{t('groupCommands.ephemeralSec' as any) || '/ephemeral [15s]'}
							</span>
							<span class="text-[10px] text-white/40 font-sans">
								{t('groupCommands.ephemeralDesc' as any) || 'Auto-delete bot msgs'}
							</span>
						</div>
						<div class="bg-black/30 rounded-[12px] p-2 border border-white/5 flex flex-col">
							<span class="text-[#3390ec] font-bold">
								{t('groupCommands.slowmodeSec' as any) || '/slowmode [sec]'}
							</span>
							<span class="text-[10px] text-white/40 font-sans">
								{t('groupCommands.slowmodeDesc' as any) || 'Rate limit delay'}
							</span>
						</div>
						<div class="bg-black/30 rounded-[12px] p-2 border border-white/5 flex flex-col">
							<span class="text-[#3390ec] font-bold">
								{t('groupCommands.purgeNum' as any) || '/purge [n]'}
							</span>
							<span class="text-[10px] text-white/40 font-sans">
								{t('groupCommands.purgeDesc' as any) || 'Bulk delete messages'}
							</span>
						</div>
					</div>
				</div>
			...
			<Show when={showLockConfirm()}>
				...
				>
					<div class="w-full max-w-md bg-[#12141C] border border-white/10 rounded-t-[28px] sm:rounded-[28px] p-6 shadow-2xl flex flex-col gap-4 animate-slide-up">
						<div class="flex items-center gap-3">
							<div
