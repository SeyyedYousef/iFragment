...
const GroupAvatar: Component<{
	...
	return (
		<div
			class={`shrink-0 rounded-[16px] bg-[#08090D] flex items-center justify-center border border-white/10 overflow-hidden shadow-inner ${props.sizeClass || 'w-14 h-14'}`}
		>
			<Show
				when={props.photoUrl && !imgFailed()}
				fallback={
					<span class={`font-black text-[#3390ec] ${props.textClass || 'text-[18px]'}`}>
						{initial()}
					</span>
				}
			>
...
export const BotManagePage: Component = () => {
	...
	const handleSubscribeCredits = async () => {
		...
		try {
			haptic.impact('heavy');
			await subscriptionApi.subscribeWithCredits(selectedGroup(), selectedPkg());
			haptic.notify('success');
			...
			setErrorMsg(e?.response?.data?.error || e?.message || 'Failed to activate with credits');
			haptic.notify('error');
		} finally {
	...
	return (
		<div
			class="min-h-screen bg-[#030303] pb-32 relative text-white overflow-x-hidden font-sans selection:bg-[#3390ec]/30"
			dir={isRtl() ? 'rtl' : 'ltr'}
		>
			{/* Ambient Top Glow */}
			...
			<div class="pt-6 pb-4 px-5 sticky top-0 bg-[#030303]/80 backdrop-blur-2xl z-40 border-b border-white/5 flex items-center justify-between shadow-sm">
				<div class="flex flex-col gap-0.5">
					<div class="flex items-center gap-2.5">
						<h1 class="text-[22px] font-black text-white tracking-tight drop-shadow-sm">
							{t('botManage.title')}
						</h1>
					...
					<Show when={bot()}>
						<div class="flex items-center gap-2 opacity-80">
							<span class="text-[12px] font-bold text-[#3390ec] font-mono tracking-tight">
								@{bot()?.bot_username}
							</span>
							<span class="w-1 h-1 rounded-full bg-white/20" />
							<span class="text-[10px] font-mono font-bold text-white/40 uppercase tracking-widest">
								ID: {bot()?.id.slice(0, 8)}
							</span>
						</div>
					</Show>
				...
				<button
					...
					class="w-10 h-10 rounded-[14px] bg-[#12141C]/80 flex items-center justify-center border border-white/10 hover:bg-white/10 active:scale-95 transition-all shrink-0 shadow-sm text-white/70"
				>
				...
				<Show when={bot()}>
					...
					<Show when={successMsg()}>
						<Motion.div
							...
							class="bg-[#00ff88]/10 border border-[#00ff88]/20 text-[#00ff88] rounded-[16px] px-4 py-3.5 flex items-center gap-2.5 text-[12px] font-bold shadow-sm"
						>
					...
					<Show when={errorMsg()}>
						<Motion.div
							initial={{ opacity: 0, y: -10 }}
							animate={{ opacity: 1, y: 0 }}
							class="bg-[#ff4a4a]/10 border border-[#ff4a4a]/20 text-[#ff4a4a] rounded-[16px] px-4 py-3.5 flex items-center gap-2.5 text-[12px] font-bold shadow-sm"
						>
					...
					<div class="grid grid-cols-2 gap-3.5">
						<div class="bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] border border-white/5 p-4.5 flex flex-col justify-center shadow-sm">
							<span class="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">
								{t('botManage.managedGroups') || 'MANAGED GROUPS'}
							</span>
							<span class="text-[26px] font-black text-white font-mono tracking-tight drop-shadow-sm">
								{bot()?.managed_groups_count || 0}
							</span>
						</div>
						<div
							class={`bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] border border-white/5 p-4.5 flex flex-col justify-center shadow-sm relative overflow-hidden ${bot()?.subscription_status === 'pro' ? 'border-amber-400/20' : ''}`}
						>
							<Show when={bot()?.subscription_status === 'pro'}>
								<div class="absolute -right-6 -top-6 w-20 h-20 bg-amber-400/10 blur-2xl rounded-full pointer-events-none" />
							</Show>
							<span class="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">
								{t('botManage.subscription' as any) || 'SUBSCRIPTION'}
							</span>
							<span
								class={`text-[20px] font-black uppercase tracking-wider drop-shadow-sm ${bot()?.subscription_status === 'pro' ? 'text-amber-400' : 'text-white/60'}`}
							>
								{bot()?.subscription_status === 'pro'
									? t('common.pro' as any) || 'PRO'
									: t('common.free' as any) || 'FREE'}
							</span>
						</div>
					</div>
					...
					<Motion.div
						...
						animate={{ opacity: 1, y: 0 }}
						class="bg-[#12141C]/80 backdrop-blur-xl rounded-[28px] border border-white/5 overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.3)]"
					>
						<div class="p-6 flex flex-col gap-6">
							<div class="flex items-center gap-4">
								<div class="w-14 h-14 rounded-[18px] bg-gradient-to-br from-[#3390ec]/20 to-[#3390ec]/5 flex items-center justify-center border border-[#3390ec]/30 shadow-inner shrink-0">
									<span class="material-symbols-outlined text-[#3390ec] text-[30px] drop-shadow-md">
										rocket_launch
									</span>
								</div>
								<div class="flex flex-col min-w-0">
									<h3 class="text-[18px] font-black text-white leading-tight mb-1 truncate">
										{t('botManage.setupTitle') || 'Setup Bot'}
									</h3>
									<p class="text-[12px] font-medium text-white/50 leading-relaxed">
										{t('botManage.setupSubtitle') || 'Follow these steps to connect your group.'}
									</p>
								</div>
							</div>
							...
							<div class="space-y-4">
								<div class="flex items-start gap-3.5 bg-[#08090D] p-3.5 rounded-[16px] border border-white/5 shadow-inner">
									<div class="w-7 h-7 rounded-[10px] bg-white/10 text-white text-[12px] font-black flex items-center justify-center shrink-0 border border-white/10">
										1
									</div>
									<p class="text-[12px] text-white/60 leading-relaxed pt-0.5">
										<span class="text-white font-bold">
											{t('botManage.step1Title') || 'Add Bot'}:
										</span>{' '}
										{t('botManage.step1Desc') || 'Invite the bot to your group.'}
									</p>
								</div>
								<div class="flex items-start gap-3.5 bg-[#08090D] p-3.5 rounded-[16px] border border-white/5 shadow-inner">
									<div class="w-7 h-7 rounded-[10px] bg-white/10 text-white text-[12px] font-black flex items-center justify-center shrink-0 border border-white/10">
									...
									<p class="text-[12px] text-white/60 leading-relaxed pt-0.5">
										<span class="text-white font-bold">
											{t('botManage.step2Title') || 'Grant Rights'}:
										</span>{' '}
										{t('botManage.step2Desc') || 'Make the bot an admin with specific rights.'}
									</p>
								</div>
							</div>
							...
							<button
								...
								class="w-full h-14 bg-gradient-to-r from-[#3390ec] to-[#2b7bc9] hover:from-[#2b7bc9] hover:to-[#3390ec] text-white rounded-[16px] font-black text-[14px] uppercase tracking-widest shadow-[0_8px_20px_rgba(51,144,236,0.3)] active:scale-95 transition-all flex items-center justify-center gap-2 border border-white/10 mt-1"
							>
								<span class="material-symbols-outlined text-[20px]">person_add</span>{' '}
								{t('botManage.inviteBtn') || 'INVITE TO GROUP'}
							</button>
						</div>
					</Motion.div>
					...
					<div class="flex flex-col gap-4 mt-2">
						<div class="flex items-center gap-2 px-1 mb-1">
							...
							<h2 class="text-[13px] font-black text-white/80 uppercase tracking-widest">
								{t('botManage.connectedGroups') || 'CONNECTED GROUPS'}
							</h2>
						</div>
							...
							<Show when={!groups.loading && (!groups() || groups()!.length === 0)}>
								<div class="bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] p-10 border border-white/5 flex flex-col items-center justify-center text-center gap-3 shadow-sm border-dashed">
									<div class="w-16 h-16 rounded-[20px] bg-white/5 flex items-center justify-center border border-white/10 mb-2">
										<span class="material-symbols-outlined text-white/30 text-[32px]">
											group_off
										</span>
									</div>
									<p class="text-white/40 text-[13px] font-bold tracking-wide">
										{t('botManage.noGroups') || 'No groups connected yet.'}
									</p>
								</div>
							</Show>
								...
								{(group: ManagedGroup, i) => {
									...
									return (
										<Motion.div
											...
											class="bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] p-4.5 border border-white/5 flex flex-col gap-4 hover:border-white/15 transition-all shadow-sm"
										>
											<div class="flex items-center justify-between">
												...
												<div class="flex items-center gap-3.5 overflow-hidden flex-1 pr-2">
													...
													<div class="flex flex-col overflow-hidden">
														<h3 class="text-[15px] font-black text-white leading-tight mb-1 truncate">
															{group.chat_title}
														</h3>
														<div class="flex items-center gap-1.5 opacity-60">
															<span class="material-symbols-outlined text-[14px]">group</span>
															<span class="text-[11px] font-mono font-bold pt-0.5">
												...
												<div class="flex flex-col items-end shrink-0 gap-1.5">
													<button
														...
														class="w-8 h-8 rounded-[10px] flex items-center justify-center hover:bg-[#ff4a4a]/10 text-white/30 hover:text-[#ff4a4a] transition-colors border border-transparent hover:border-[#ff4a4a]/20"
														aria-label="Delete"
													>
														<span class="material-symbols-outlined text-[18px]">delete</span>
													</button>
													<span
														class={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-[8px] border shadow-sm ${isPaid ? 'text-[#00ff88] border-[#00ff88]/30 bg-[#00ff88]/10' : isTrial ? 'text-amber-400 border-amber-400/30 bg-amber-400/10' : 'text-[#ff4a4a] border-[#ff4a4a]/30 bg-[#ff4a4a]/10'}`}
													>
														{isPaid
															? 'ACTIVE'
															: isTrial
													...
													<Show when={endDateStr && !isExpired}>
														<span class="text-[10px] text-white/40 font-mono font-bold mt-0.5 whitespace-nowrap">
											...
											<div class="flex gap-2.5 w-full pt-1">
												<Show when={!isExpired}>
													<button
														...
														class="flex-1 h-12 rounded-[14px] text-[12px] font-black uppercase tracking-widest transition-all bg-white/5 text-white border border-white/10 hover:bg-white/10 active:scale-95 shadow-sm"
													>
												...
												<button
													...
													class={`flex-1 h-12 rounded-[14px] text-[12px] font-black uppercase tracking-widest transition-all shadow-sm active:scale-95 border ${isPaid ? 'bg-[#08090D] text-white/60 border-white/5 hover:bg-white/5' : 'bg-[#3390ec] text-white border-transparent shadow-[0_8px_20px_rgba(51,144,236,0.3)] hover:bg-[#2b7ec9]'}`}
												>
													{isPaid
														? t('botManage.extendSub' as any) || 'EXTEND'
														: t('botManage.buySubscription' as any) || 'SUBSCRIBE'}
												</button>
											</div>
										</Motion.div>
									);
			...
			<Show when={showSubscription()}>
				...
				>
					<Motion.div
						...
						class="w-full max-h-[92vh] bg-[#12141C] rounded-t-[32px] border-t border-white/10 p-6 overflow-y-auto no-scrollbar shadow-[0_-30px_80px_rgba(0,0,0,0.8)] relative"
						dir={isRtl() ? 'rtl' : 'ltr'}
					>
						<div class="w-12 h-1.5 bg-white/10 rounded-full mx-auto mb-6" />

						<Show when={successMsg()}>
							<div class="bg-[#00ff88]/10 border border-[#00ff88]/30 text-[#00ff88] rounded-[16px] px-4 py-3.5 flex items-center gap-2.5 text-[12px] font-bold mb-5 shadow-sm">
								<span class="material-symbols-outlined text-[20px]">check_circle</span>
								{successMsg()}
							</div>
						</Show>
						<Show when={errorMsg()}>
							<div class="bg-[#ff4a4a]/10 border border-[#ff4a4a]/30 text-[#ff4a4a] rounded-[16px] px-4 py-3.5 flex items-center gap-2.5 text-[12px] font-bold mb-5 shadow-sm">
						...
						{paymentStep() === 'package' ? (
							<>
								<div class="flex flex-col items-center text-center gap-2 mb-6">
									<div class="w-16 h-16 rounded-[20px] bg-gradient-to-br from-[#3390ec]/20 to-transparent border border-white/10 flex items-center justify-center shadow-[inset_0_2px_10px_rgba(255,255,255,0.05),0_10px_30px_rgba(51,144,236,0.15)] mb-1">
										<span class="material-symbols-outlined text-[36px] text-[#3390ec] drop-shadow-md">
											workspace_premium
										</span>
									</div>
									<h3 class="text-[22px] font-black text-white leading-tight tracking-tight">
										{t('botManage.choosePackage' as any) || 'Choose a Plan'}
									</h3>
									<p class="text-[13px] font-medium text-white/50 max-w-xs">
										{t('botManage.selectPlan' as any) || 'Select a monthly plan for your group.'}
									</p>
								</div>

								{/* Credit Balance Hub */}
								<div class="flex items-center justify-between w-full bg-gradient-to-r from-[#121829] to-[#0a0d14] border border-[#3390ec]/30 rounded-[20px] p-3.5 mb-5 shadow-sm">
									<div class="flex items-center gap-2.5">
										<span class="text-[20px]">💎</span>
										<div class="flex flex-col text-start">
											<span class="text-[10px] font-black uppercase text-[#3390ec] tracking-wider">
										...
										{(pkg: SubscriptionPackage) => {
											...
											return (
												<button
													...
													class={`w-full rounded-[24px] p-4.5 flex items-center justify-between border-[1.5px] transition-all active:scale-[0.98] relative overflow-hidden text-start ${
														isSelected()
															? 'border-[#3390ec] bg-[#3390ec]/10 shadow-[0_8px_24px_rgba(51,144,236,0.15)]'
															: 'border-white/10 bg-[#08090D] hover:border-white/20 hover:bg-[#161b28]'
													}`}
												>
													<Show when={isSelected()}>
														<div class="absolute -right-6 -top-6 w-24 h-24 bg-[#3390ec]/10 rounded-full blur-2xl pointer-events-none" />
													</Show>
													<Show when={pkg.badge}>
														<div
													...
													<div class="flex flex-col items-start gap-1 z-10 relative">
														<div class="flex items-center gap-2">
															<span
																class={`text-[17px] font-black ${isSelected() ? 'text-white' : 'text-white/90'}`}
															>
																{pkg.name}
															</span>
													...
													<div class="flex flex-col items-end gap-0.5 z-10 relative">
														<div class="flex items-baseline gap-1" dir="ltr">
															<span class="text-[24px] font-black font-mono tracking-tight text-white">
																{credits}
															</span>
											...
											<div class="space-y-3">
												...
												>
													<button
														...
														class="w-full h-14 bg-gradient-to-r from-[#3390ec] to-[#2b7ec9] text-white rounded-[18px] font-black text-[15px] tracking-wide transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-[0_8px_25px_rgba(51,144,236,0.35)] active:scale-95 border border-white/15"
													>
														<span>💎</span>
							...
							<>
								<div class="flex items-center gap-4 mb-6">
									<button
										...
										class="w-10 h-10 rounded-[12px] bg-white/5 flex items-center justify-center border border-white/10 hover:bg-white/10 active:scale-95 transition-all text-white/70"
									>
									...
									<div class="flex flex-col">
										...
										<p class="text-[12px] font-medium text-white/50">
											{t('botManage.paymentMethodDesc' as any) ||
												'Choose your options and pay with Telegram Stars'}
										</p>
									</div>
								</div>
												...
												<div class="bg-[#08090D] rounded-[20px] p-5 border border-white/5 flex items-center justify-between shadow-inner">
													<div class="flex flex-col gap-1">
														...
														<span class="text-[12px] font-mono text-white/50">
															${pkg.price_per_month.toFixed(2)}/mo
														</span>
													</div>
													<div class="flex flex-col items-end gap-1">
														...
														<span class="text-[11px] font-bold text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded-[6px] border border-amber-400/20">
						...
						<Show when={isProcessing()}>
							<div class="absolute inset-0 bg-[#12141C]/90 backdrop-blur-md z-30 flex flex-col items-center justify-center rounded-t-[32px]">
								...
								<span class="text-[14px] font-black uppercase tracking-widest text-white animate-pulse">
									{t('botManage.processing')}
								</span>
							</div>
						</Show>
					</Motion.div>
			...
			<Show when={groupToDelete()}>
				<Motion.div
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
					class="fixed inset-0 bg-[#030303]/90 backdrop-blur-md z-[150] flex items-center justify-center px-5"
					onClick={(e) => {
				...
				>
					<Motion.div
						...
						class="w-full max-w-sm max-h-[85vh] overflow-y-auto no-scrollbar bg-[#12141C] rounded-[32px] border border-white/10 p-7 flex flex-col items-center text-center shadow-[0_20px_60px_rgba(0,0,0,0.8)] relative"
					>
						...
						<p class="text-[13px] text-white/50 mb-8 leading-relaxed font-medium px-2 relative z-10">
							{t('botManage.deleteConfirmDesc' as any) ||
								'This action cannot be undone. All settings and management features will be disabled.'}
						</p>
						...
						<div class="w-full flex flex-col gap-3 relative z-10">
							<button
								...
								class="w-full h-14 rounded-[16px] font-black text-[14px] uppercase tracking-widest bg-[#ff4a4a] hover:bg-[#ff3b30] text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-[0_8px_24px_rgba(255,74,74,0.3)] active:scale-95 border border-white/10"
							>
								<Show
								...
								>
									...
									{t('managedBots.delete' as any) || 'YES, REMOVE'}
								</Show>
							</button>
							<button
								type="button"
								...
								disabled={isDeletingGroup()}
								class="w-full h-14 rounded-[16px] font-bold text-[14px] uppercase tracking-widest bg-transparent hover:bg-white/5 text-white/60 hover:text-white transition-all disabled:opacity-50 active:scale-95 border border-transparent hover:border-white/5"
							>
