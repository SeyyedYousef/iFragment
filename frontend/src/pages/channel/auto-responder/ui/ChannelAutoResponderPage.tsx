    ...
    <div class="min-h-screen bg-[#0f1014] pb-28 relative overflow-x-hidden text-white">
      ...
      <div class="px-5 pt-6 pb-4 bg-[#0f1014]/80 backdrop-blur-md sticky top-0 z-30 border-b border-[#1c1c1c] flex items-center justify-between gap-3">
        <div class="flex items-center gap-2 overflow-hidden flex-1">
          ...
          <div class="flex flex-col overflow-hidden">
            <h1 class="text-[18px] font-black text-white leading-tight truncate">{t('channelAutoResponder.title') || 'Auto Responder'}</h1>
            <span class="text-[12px] text-on-surface-variant truncate">{t('channelAutoResponder.subtitle') || 'Manage automated replies & comments'}</span>
          </div>
        </div>
      ...
      <div class="px-5 pt-6 flex flex-col gap-6 pb-24">
        ...
        <Show when={!isCreating()}>
          <Motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} class="flex flex-col gap-6">
            ...
            <div class="bg-[#1c1c1c] rounded-3xl border border-[#2a2a2a] p-5 flex flex-col gap-4">
               <SettingsSection
                  ...
                  description={t('channelAutoResponder.firstCommentDesc') || 'Automatically comment on your own posts.'}
               ...
               <Show when={autoFirstComment()}>
                  <div class="flex flex-col gap-4 pt-4 border-t border-[#2a2a2a]">
                     <SelectField 
                        ...
                        options={[
                           { value: 'fixed', label: t('channelAutoResponder.modeFixed') || 'Fixed Text' },
                           { value: 'rotating', label: t('channelAutoResponder.modeRotating') || 'Rotating Texts (Random)' },
                           { value: 'ai', label: t('channelAutoResponder.modeAi') || 'AI Generated (Context Aware)' }
                        ]}
                     />
                     
                     <Show when={commentMode() === 'fixed'}>
                        <div class="flex flex-col gap-2">
                           <label class="text-[13px] font-bold text-white">{t('channelAutoResponder.commentText') || 'Comment Text'}</label>
                           <textarea 
                              value={fixedComment()} 
                              onInput={(e) => setFixedComment(e.currentTarget.value)}
                              placeholder={t('channelAutoResponder.commentTextPlaceholder') || 'e.g. Let us know what you think!'}
                     ...
                     <Show when={commentMode() === 'rotating'}>
                        <div class="flex flex-col gap-3">
                           <label class="text-[13px] font-bold text-white">{t('channelAutoResponder.rotatingTexts') || 'Rotating Texts'}</label>
                           <div class="flex flex-col gap-2">
                              <For each={rotatingTexts()}>
                                 {(text, i) => (
                                    <div class="flex items-center justify-between bg-[#2c2c2e] px-3 py-2 rounded-lg border border-[#3a3a3c]">
                                       ...
                                       <button onClick={() => handleRemoveRotatingText(i())} class="w-6 h-6 flex items-center justify-center text-[#ff3b30] bg-[#ff3b30]/10 rounded-full">
                           ...
                           <div class="flex gap-2">
                              <input 
                                 ...
                                 placeholder={t('channelAutoResponder.addRotatingTextPlaceholder') || 'Add new text...'}
                                 class="bg-[#2c2c2e] text-white text-[14px] rounded-lg px-3 py-2 flex-1 focus:outline-none focus:ring-2 focus:ring-[#32ade6]"
                              />
                              <button onClick={handleAddRotatingText} disabled={!newRotatingText().trim()} class="px-4 bg-[#32ade6] text-black font-bold rounded-lg hover:bg-[#2b96c8] disabled:opacity-50">
                     ...
                     <Show when={commentMode() === 'ai'}>
                        <div class="bg-gradient-to-r from-[#32ade6]/10 to-transparent p-3 rounded-xl border border-[#32ade6]/30 flex flex-col gap-1">
                           ...
                           <span class="text-[12px] text-on-surface-variant">{t('channelAutoResponder.aiAutoCommentDesc') || 'The AI will read your post and generate a highly relevant first comment automatically.'}</span>
                        </div>
                     </Show>
                     ...
                     <div class="flex flex-col gap-2 pt-2 border-t border-[#2a2a2a]">
                        <SelectField 
                           ...
                           options={[
                              ...
                              { value: 'like_set', label: t('channelAutoResponder.btnLikeSet') || '👍👎 Like/Dislike Set' },
                              { value: 'share_set', label: t('channelAutoResponder.btnShareSet') || 'Share Set' }
                           ]}
                        />
                     </div>
                  </div>
            ...
            <div class="bg-[#1c1c1c] rounded-3xl border border-[#2a2a2a] p-5 flex flex-col gap-4">
               <SettingsSection
                  ...
                  description={t('channelAutoResponder.welcomeMessageDesc') || 'Send a message when someone joins the discussion group.'}
                  ...
                  <div class="flex flex-col gap-4 pt-4 border-t border-[#2a2a2a]">
                     <div class="flex gap-2">
                        <div class="flex-1 flex flex-col gap-2">
                           <label class="text-[13px] font-bold text-white">{t('channelAutoResponder.welcomeDelay') || 'Delay (Seconds)'}</label>
                           <input 
                              type="number" 
                              value={welcomeDelay()} 
                     ...
                     <div class="flex flex-col gap-2">
                        <label class="text-[13px] font-bold text-white flex justify-between">
                           ...
                           <span class="text-[#32ade6] text-[11px] font-normal">{t('channelAutoResponder.useName') || 'Use $name'}</span>
                        </label>
                        <textarea 
                           value={welcomeText()} 
                           onInput={(e) => setWelcomeText(e.currentTarget.value)}
                           placeholder={t('channelAutoResponder.welcomeTextPlaceholder') || 'Welcome $name to the group!'}
                           class="bg-[#2c2c2e] text-white text-[15px] rounded-xl px-4 py-3 w-full min-h-[80px] focus:outline-none focus:ring-2 focus:ring-[#32ade6] resize-none"
                        />
                     </div>
                  </div>
            ...
            <div class="flex flex-col gap-3">
               <h2 class="text-[16px] font-bold text-white flex items-center justify-between">
                  ...
                  <span class="bg-[#32ade6] text-black px-2 py-0.5 rounded-md text-[11px]">{rules().length}</span>
               </h2>
               <Show when={rules().length === 0}>
                  <div class="bg-[#1c1c1c] rounded-3xl border border-[#2a2a2a] p-5 flex flex-col items-center text-center gap-3">
                    ...
                    <h2 class="text-[16px] font-bold text-white">{t('channelAutoResponder.noRules') || 'No Rules'}</h2>
                    <p class="text-[13px] text-[#8e8e93]">{t('channelAutoResponder.keywordRepliesDesc') || 'Auto-reply to specific words in the comments.'}</p>
                  </div>
               </Show>
                      ...
                      {(rule) => (
                         <div class="bg-[#1c1c1c] rounded-2xl border border-[#2a2a2a] p-4 flex items-center justify-between gap-3">
                            <div class="flex flex-col flex-1 min-w-0">
                              ...
                              <span class="text-[12px] text-[#32ade6] uppercase tracking-wide font-bold">{getLocalizedMatch(rule.match)}</span>
                            </div>
                            <button 
                               onClick={() => {
                                  hapticFeedback.impactOccurred('light');
               ...
               <button onClick={() => setIsCreating(true)} class="mt-2 h-12 bg-[#32ade6] text-black font-bold rounded-xl hover:bg-[#2b96c8] transition-colors flex items-center justify-center gap-2">
        ...
        <Show when={isCreating()}>
          <Motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} class="flex flex-col gap-4">
            ...
            <div class="bg-[#1c1c1c] rounded-3xl border border-[#2a2a2a] p-5 flex flex-col gap-4">
              <h2 class="text-[16px] font-bold text-white">{t('channelAutoResponder.addRule') || 'Add Keyword Rule'}</h2>
              
              <div class="flex flex-col gap-2">
                <label class="text-[13px] font-bold text-white">{t('channelAutoResponder.triggerKey') || 'Keywords'}</label>
                <p class="text-[11px] text-on-surface-variant -mt-1">{t('channelAutoResponder.commaSeparated') || 'Comma separated'}</p>
                <input 
                  type="text" 
                  value={keywords()} 
              ...
              <SelectField 
                 ...
                 options={[
                   ...
                   { value: 'contains', label: t('channelAutoResponder.matchContains') || 'Contains' },
            ...
            <div class="bg-[#1c1c1c] rounded-3xl border border-[#2a2a2a] p-5 flex flex-col gap-4">
              <div class="flex flex-col gap-2">
                <label class="text-[13px] font-bold text-white">{t('channelAutoResponder.replyText') || 'Reply Text'}</label>
                <textarea 
                  value={replyText()} 
                  onInput={(e) => setReplyText(e.currentTarget.value)}
                  placeholder={t('channelAutoResponder.replyPlaceholder') || 'Type your automated reply here...'}
              ...
              <div class="flex items-center justify-between gap-3 bg-gradient-to-r from-[#32ade6]/10 to-transparent p-3 rounded-xl border border-[#32ade6]/20">
                <div class="flex flex-col flex-1 min-w-0">
                  ...
                  <span class="text-[11px] text-[#8e8e93] leading-snug mt-1">{t('channelAutoResponder.useAiDesc') || 'Allow AI to slightly tweak the response for a natural tone.'}</span>
                </div>
