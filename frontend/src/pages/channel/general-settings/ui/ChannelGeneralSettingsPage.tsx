...
export const ChannelGeneralSettingsPage: Component = () => {
		...
		async (channelId) => {
			try {
				...
				const merged: ChannelConfig = {
					...
					channelName:
						(apiGeneral as any).name ||
						(apiGeneral as any).channelName ||
						defaultConfig.channelName,
					channelBio:
						(apiGeneral as any).description ||
						(apiGeneral as any).channelBio ||
						defaultConfig.channelBio,
					channelPhotoUrl:
						(apiGeneral as any).photo ||
						(apiGeneral as any).channelPhotoUrl ||
						defaultConfig.channelPhotoUrl,
					channelUsername:
						(apiGeneral as any).username ||
					...
					hideHistory:
						(apiGeneral as any).hideChatHistory ??
					...
					inputChannelBio:
						(inputGeneral as any).description || (inputGeneral as any).channelBio || '',
					inputChannelPhotoUrl:
						(inputGeneral as any).photo || (inputGeneral as any).channelPhotoUrl || '',
					inputChannelUsername:
						(inputGeneral as any).username || (inputGeneral as any).channelUsername || '',
				};
	...
	const handleBack = async () => {
		...
		if (isDirty()) {
			...
			const confirmDiscard = await showConfirm(t('channelSettings.unsavedChangesConfirm'));
			if (confirmDiscard) {
				setIsDirty(false);
				navigate(-1);
			}
				...
				<div class="flex items-center gap-3.5 overflow-hidden flex-1">
					...
					<div class="flex flex-col overflow-hidden">
						<div class="flex items-center gap-2.5">
							...
							<Show when={isDirty()}>
								<span
									class="w-2 h-2 rounded-full bg-[#3390ec] animate-pulse shrink-0 shadow-[0_0_8px_rgba(51,144,236,0.8)]"
				...
				>
						...
						>
							...
							<div class="flex items-center gap-3 relative z-10 border-b border-white/5 pb-3">
								<div class="w-10 h-10 rounded-[12px] bg-[#3390ec]/15 flex items-center justify-center border border-[#3390ec]/30 shadow-inner shrink-0">
									<span class="material-symbols-outlined text-[#3390ec] text-[20px] rtl:-scale-x-100">
										input
									</span>
								</div>
								<h2 class="text-[14px] font-black text-white tracking-tight uppercase tracking-widest">
									{t('channelSettings.inputChannelIdentity')}
								</h2>
							</div>
							...
							<div class="flex flex-col items-center sm:flex-row gap-5 relative z-10">
								<div class="relative group shrink-0">
									<div class="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-[#3390ec]/20 to-[#2b96c8]/5 flex items-center justify-center border-2 border-white/10 cursor-pointer shadow-inner relative z-10">
										<Show
											...
											fallback={
												<span class="text-[28px] font-black text-[#3390ec] drop-shadow-md">
										...
										<input
											...
											class="absolute inset-0 opacity-0 cursor-pointer w-full h-full text-[0px] font-[0px] z-20"
											onChange={(e) => {
								...
								<div class="flex flex-col gap-3 flex-1 w-full min-w-0">
									<div class="flex flex-col gap-1.5">
										<div class="text-[10px] font-black uppercase tracking-widest text-white/40 px-1">
									...
									<div class="flex flex-col gap-1.5">
										<div class="text-[10px] font-black uppercase tracking-widest text-white/40 px-1">
											{t('channelSettings.channelUsername')}
										</div>
										<input
											type="text"
											...
											readOnly
											placeholder="@channel"
											class="bg-[#08090D] border border-white/5 text-white/60 font-mono text-[13px] rounded-[16px] px-4 py-3.5 w-full focus:outline-none cursor-not-allowed shadow-inner"
											dir="ltr"
										/>
									</div>
								</div>
					...
					>
						...
						<div class="flex items-center gap-3 relative z-10 border-b border-white/5 pb-3">
							...
							<h2 class="text-[14px] font-black text-white tracking-tight uppercase tracking-widest">
								{t('channelSettings.outputChannelIdentity')}
							</h2>
						</div>
							...
							<div class="relative group shrink-0">
								<div class="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-[#10b981]/20 to-[#059669]/5 flex items-center justify-center border-2 border-white/10 cursor-pointer shadow-inner relative z-10">
									<Show
										...
										fallback={
											<span class="text-[28px] font-black text-[#10b981] drop-shadow-md">
					...
					<Motion.div
						...
						class="bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] border border-white/5 p-2 shadow-sm flex flex-col gap-2"
					>
						<SelectField
					...
					<Motion.div
						...
						class="bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] border border-white/5 p-5 flex flex-col gap-4 shadow-sm relative overflow-hidden mt-1"
					>
						<SettingsSection
						...
						<Show when={config.signMessages}>
							<div class="flex flex-col gap-1.5 pt-3 border-t border-white/5 mt-1 relative z-10">
								...
								<div class="relative">
									<span class="absolute right-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-white/30 text-[20px] pointer-events-none">
										draw
									</span>
									<input
										type="text"
					...
					<Motion.div
						...
						class="bg-[#12141C]/80 backdrop-blur-xl rounded-[28px] border border-white/5 p-5 flex flex-col shadow-sm relative overflow-hidden mt-1"
					>
