        ...
        <div class="px-5 pt-6 flex flex-col gap-6">
          ...
          <div class="flex bg-[#1c1c1c] p-1 rounded-xl border border-[#2a2a2a]">
             <For each={['7d', '30d', '90d', 'Custom']}>
                {(range) => (
                   ...
                   >
                      {range === '7d' ? t('channelAnalytics.range7d') :
                       range === '30d' ? t('channelAnalytics.range30d') :
                       range === '90d' ? t('channelAnalytics.range90d') :
          ...
          <div class="grid grid-cols-2 gap-3">
             ...
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-1">
               <span class="material-symbols-outlined text-[#8e8e93] text-[20px] mb-1">trending_up</span>
               <h3 class={`text-2xl font-black ${getErrColor(analytics()?.summary.engagement_rate || 6.2)}`}>
                  {analytics()?.summary.engagement_rate || 6.2}%
               </h3>
               <p class="text-[11px] text-[#8e8e93] font-medium">{t('channelAnalytics.avgEngagement') || 'Engagement Rate'}</p>
             </div>
             ...
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-1 relative overflow-hidden">
               ...
               <span class="material-symbols-outlined text-[#bf5af2] text-[20px] mb-1">workspace_premium</span>
               <div class="flex items-end gap-2">
                  <h3 class="text-2xl font-black text-white">A+</h3>
                  <span class="text-[12px] font-bold text-[#bf5af2] mb-1">Top 5%</span>
               </div>
               <p class="text-[11px] text-[#8e8e93] font-medium">{t('channelAnalytics.citationIndex')}</p>
             </div>
             ...
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-2 col-span-2">
               <div class="flex items-center justify-between">
                  <div class="flex flex-col">
                     <span class="text-[11px] text-[#8e8e93] font-medium">{t('channelAnalytics.followersGrowth') || 'Member Growth'}</span>
                     <div class="flex items-end gap-2">
               ...
               <div class="h-16 w-full flex items-end gap-1 mt-2">
                  <For each={growthData}>
                     {(point) => (
                        <div class="flex-1 bg-[#34c759]/20 hover:bg-[#34c759] transition-colors rounded-t-sm" style={{ height: `${(point / maxGrowth) * 100}%` }}></div>
                     )}
                  </For>
               </div>
             </div>
             ...
             <Motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} class="bg-[#1c1c1c] rounded-3xl border border-[#2a2a2a] p-4 flex flex-col gap-2 col-span-2">
               <div class="flex items-center justify-between">
                  <div class="flex flex-col">
                     <span class="text-[11px] text-[#8e8e93] font-medium">{t('channelAnalytics.viewsPerPostAvg') || 'Views Per Post (Avg)'}</span>
                  ...
                  <For each={postViewsData}>
                     {(views, idx) => (
                        <div class="w-full flex flex-col items-center justify-end h-full gap-1 group">
                           <div class={`w-full rounded-t-md transition-all ${idx() === 5 ? 'bg-[#32ade6] shadow-[0_0_10px_rgba(50,173,230,0.3)]' : 'bg-[#2a2a2a] group-hover:bg-[#3a3a3c]'}`} style={{ height: `${(views / maxViews) * 100}%` }}></div>
                        </div>
                     )}
                  </For>
               </div>
             ...
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-1 col-span-2 relative overflow-hidden">
                ...
                <div class="flex items-center gap-3">
                   ...
                   <div class="flex flex-col">
                      <span class="text-[13px] font-bold text-white">{t('channelAnalytics.bestPostingTime')}</span>
                      <span class="text-[11px] text-[#8e8e93]">{t('channelAnalytics.bestPostingTimeDesc')}</span>
                   </div>
             ...
             <div class="bg-gradient-to-br from-[#32ade6]/10 to-transparent p-4 rounded-3xl border border-[#32ade6]/30 flex flex-col gap-2 col-span-2">
                <div class="flex items-center gap-2">
                   ...
                   <span class="text-[14px] font-bold text-white">{t('channelAnalytics.contentAnalysis')}</span>
                </div>
                <p class="text-[13px] text-on-surface-variant leading-relaxed" innerHTML={t('channelAnalytics.aiInsightsText')} />
             </div>

             {/* Mentions */}
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-1">
                <span class="material-symbols-outlined text-[#34c759] text-[20px] mb-1">alternate_email</span>
                <h3 class="text-2xl font-black text-white">142</h3>
                <p class="text-[11px] text-[#8e8e93] font-medium">{t('channelAnalytics.mentionsIn')}</p>
             </div>
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-1">
                <span class="material-symbols-outlined text-[#ff3b30] text-[20px] mb-1">forward_to_inbox</span>
                <h3 class="text-2xl font-black text-white">28</h3>
                <p class="text-[11px] text-[#8e8e93] font-medium">{t('channelAnalytics.mentionsOut')}</p>
             </div>
                      ...
                      {(geo) => (
                         <div class="flex items-center gap-3">
                            <span class="text-[13px] font-medium text-white w-16">
                               {geo.country === 'Iran' ? t('channelAnalytics.geoIran') :
                                geo.country === 'USA' ? t('channelAnalytics.geoUSA') :
                                geo.country === 'Germany' ? t('channelAnalytics.geoGermany') :
                                geo.country === 'UK' ? t('channelAnalytics.geoUK') :
                                t('channelAnalytics.geoOther')}
                            </span>
                            <div class="flex-1 h-2 bg-[#2a2a2a] rounded-full overflow-hidden">
                               <div class="h-full rounded-full" style={{ width: `${geo.percent}%`, 'background-color': geo.color }}></div>
                            </div>
                            <span class="text-[12px] font-bold text-[#8e8e93] w-8 text-right">{geo.percent}%</span>
                         </div>
                      )}
                   </For>
                </div>
             ...
             <div class="bg-[#1c1c1c] p-4 rounded-3xl border border-[#2a2a2a] flex flex-col gap-3 col-span-2 mt-2">
                ...
                <div class="flex gap-2">
                   ...
                   >
                      <Show when={!isComparing()} fallback={<span class="w-5 h-5 border-2 border-black/30 border-t-black rounded-full animate-spin" />}>
                      ...
                      {(channel) => (
                         <div class="flex items-center justify-between p-3 bg-[#2c2c2e] rounded-xl border border-[#3a3a3c]">
                            <div class="flex flex-col">
                               ...
                               <span class="text-[11px] text-[#8e8e93]">{channel.members} {t('channelAdmins.members') || 'members'}</span>
                            </div>
                            <div class="flex flex-col items-end">
                               <span class="text-[12px] font-bold text-[#32ade6]">{channel.match} {t('channelAnalytics.match') || 'Match'}</span>
                            </div>
                         </div>
                      )}
                   </For>
