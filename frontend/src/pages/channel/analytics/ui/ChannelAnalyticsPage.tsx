	...
	return (
		...
		>
				...
				<div class="grid grid-cols-2 gap-3.5">
					...
					<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 hover:border-white/15 rounded-[24px] p-4.5 flex flex-col justify-center transition-all shadow-sm group">
						<div class="flex items-center gap-2 mb-2">
							<div
								class={`w-8 h-8 rounded-[10px] flex items-center justify-center border shadow-inner ${getErrBg(analytics()?.summary?.engagement_rate ?? 0)}`}
							>
								<span
									class={`material-symbols-outlined text-[16px] ${getErrColor(analytics()?.summary?.engagement_rate ?? 0)}`}
								>
									trending_up
								</span>
							</div>
							<span class="text-[10px] font-black uppercase tracking-widest text-white/40">
								{t('channelAnalytics.errRate')}
							</span>
						</div>
						<h3
							class={`text-[28px] font-black font-mono tracking-tight drop-shadow-sm ${getErrColor(analytics()?.summary?.engagement_rate ?? 0)}`}
						>
					...
					>
						...
						<div class="flex items-center gap-2 mb-2 relative z-10">
							<div class="w-8 h-8 rounded-[10px] bg-[#06b6d4]/10 flex items-center justify-center border border-[#06b6d4]/30 shadow-inner shrink-0">
								<span class="material-symbols-outlined text-[#06b6d4] text-[16px]">
									workspace_premium
								</span>
							</div>
							<span class="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-1">
								{t('channelAnalytics.citationIndex')}{' '}
								<span class="material-symbols-outlined text-[14px]">info</span>
							</span>
						</div>
						<div class="flex items-end gap-2 relative z-10">
							<h3 class="text-[28px] font-black font-mono text-white tracking-tight drop-shadow-sm">
					...
					<Motion.div
						...
						class="col-span-2 bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[24px] p-5 flex flex-col gap-4 shadow-sm relative overflow-hidden"
					>
						...
						<div class="flex items-center justify-between relative z-10">
							<div class="flex flex-col gap-0.5">
								<span class="text-[11px] font-black uppercase tracking-widest text-white/40 flex items-center gap-1.5">
									<span class="material-symbols-outlined text-[#10b981] text-[16px]">groups</span>{' '}
									{t('channelAnalytics.memberGrowth')}
								</span>
								<div class="flex items-end gap-2.5">
									<h3 class="text-[26px] font-black text-white font-mono tracking-tight drop-shadow-sm">
										+{(analytics()?.summary?.new_members || 0).toLocaleString('en-US')}
									</h3>
									<Show when={analytics()?.summary?.new_members_today}>
										<span class="text-[10px] font-black text-[#10b981] bg-[#10b981]/10 px-2 py-0.5 rounded-[6px] border border-[#10b981]/20 uppercase tracking-widest mb-1.5 shadow-sm">
											+{analytics()?.summary?.new_members_today || 0} {t('channelAnalytics.today')}
										</span>
									</Show>
								</div>
							</div>
							...
							<For each={growthData()}>
								{(point) => (
									<div class="flex-1 flex flex-col justify-end h-full group">
										<div
											class="w-full bg-gradient-to-t from-[#10b981]/20 to-[#10b981]/60 group-hover:to-[#10b981] transition-all duration-300 rounded-t-[4px] rounded-b-[2px]"
											style={{ height: `${Math.max(4, (point / maxGrowth()) * 100)}%` }}
										/>
									</div>
								)}
							</For>
						...
						<div class="flex items-center justify-between relative z-10">
							<div class="flex flex-col gap-0.5">
								<span class="text-[11px] font-black uppercase tracking-widest text-white/40 flex items-center gap-1.5">
									...
									{t('channelAnalytics.avgViewsPerPost')}
								</span>
								<h3 class="text-[26px] font-black text-white font-mono tracking-tight drop-shadow-sm">
						...
						<div class="h-[100px] w-full flex items-end gap-2 relative z-10 border-b border-white/5 pb-1">
							<For each={postViewsData()}>
								{(views) => (
									<div class="flex-1 flex flex-col justify-end h-full group">
										<div
											class="w-full bg-gradient-to-t from-[#3390ec]/20 to-[#3390ec]/60 group-hover:to-[#3390ec] transition-all duration-300 rounded-t-[4px] rounded-b-[2px]"
											style={{ height: `${Math.max(4, (views / maxViews()) * 100)}%` }}
										/>
									</div>
								)}
							</For>
			...
			<Show when={showCiModal()}>
				<div
					class="fixed inset-0 z-[9990] bg-[#030303]/90 backdrop-blur-2xl flex items-end justify-center px-2 pb-2"
					role="button"
					tabIndex={0}
					aria-label="Close"
					onKeyDown={(e) => {
						if (e.key === 'Escape') setShowCiModal(false);
					}}
					onClick={(e) => {
						if (e.target === e.currentTarget) setShowCiModal(false);
					}}
				>
					<Motion.div
						initial={{ y: '100%' }}
						...
						class="w-full max-w-md max-h-[85vh] overflow-y-auto no-scrollbar bg-[#12141C] border border-white/10 rounded-[32px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.8)] relative"
					>
						...
						<div class="flex items-center gap-3.5 mb-5 relative z-10">
							<div class="w-12 h-12 rounded-[16px] bg-[#06b6d4]/10 border border-[#06b6d4]/30 flex items-center justify-center shadow-inner shrink-0">
								<span class="material-symbols-outlined text-[#06b6d4] text-[24px]">
									workspace_premium
								</span>
							</div>
							<h3 class="text-[18px] font-black text-white tracking-tight">
								{t('channelAnalytics.ciModalTitle')}
							</h3>
						</div>
						...
						<div class="bg-[#08090D] border border-white/5 rounded-[20px] p-4 flex flex-col gap-3 relative z-10 shadow-inner mb-6">
							<div class="flex items-center justify-between border-b border-white/5 pb-3">
								<span class="text-[11px] font-black uppercase tracking-widest text-white/40">
									{t('channelAnalytics.currentRank') || 'Score Grade'}
								</span>
							...
							<div class="flex items-center justify-between pt-1">
								<span class="text-[11px] font-black uppercase tracking-widest text-white/40">
									{t('channelAnalytics.qualityClass')}
								</span>
								<span class="text-[12px] font-black text-[#10b981] bg-[#10b981]/10 px-2.5 py-1 rounded-[8px] border border-[#10b981]/20 shadow-sm uppercase tracking-widest">
									{t('channelAnalytics.qualityValue')}
								</span>
							</div>
						</div>
						...
						<button
							...
							class="w-full h-14 bg-white/5 hover:bg-white/10 active:scale-95 text-white rounded-[16px] font-black text-[13px] uppercase tracking-widest transition-all relative z-10 border border-white/10"
						>
