...
export const DynamicBioLessonCard: Component<{ onNavigate: () => void; isDone?: boolean }> = (
	props,
) => {
	...
	onMount(() => {
		const timer = setInterval(
			() => setMembersCount((m) => m + Math.floor(Math.random() * 5) + 1),
			1500,
		);
	...
	return (
		<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 hover:border-white/15 rounded-[28px] p-5 flex flex-col gap-4 shadow-sm relative overflow-hidden transition-all">
			<div class="absolute -right-10 -top-10 w-32 h-32 bg-cyan-500/10 blur-3xl rounded-full pointer-events-none" />
