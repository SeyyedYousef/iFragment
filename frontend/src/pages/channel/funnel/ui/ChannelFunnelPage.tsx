...
export const ChannelFunnelPage: Component = () => {
	...
	const [funnel, { mutate: mutateFunnel }] = createResource(() => params.id, (id) => channelApi.getFunnel(id));
	const [userChannels] = createResource(() => channel()?.bot_id, (botId) => (botId ? channelApi.getUserChannels(botId) : Promise.resolve([])));
	...
	return (
		<div class="min-h-screen bg-[#030303] pb-28 relative overflow-x-hidden text-white font-sans selection:bg-[#3390ec]/30" dir={isRtl() ? 'rtl' : 'ltr'}>
			...
			<div class="px-5 pt-5 flex flex-col gap-6 max-w-md mx-auto relative z-10 w-full">
				...
				<div class="bg-[#12141C]/80 backdrop-blur-xl border border-white/5 rounded-[28px] p-5 flex flex-col gap-5 shadow-sm relative overflow-hidden">
					...
					<div class="flex items-center justify-between border-b border-white/5 pb-3 relative z-10">
						...
						<span class={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-[8px] shadow-sm border ${funnel() ? 'bg-[#10b981]/10 text-[#10b981] border-[#10b981]/20' : 'bg-white/5 text-white/40 border-white/10'}`}>
					...
					<div class="grid grid-cols-3 gap-2 text-center pt-1 relative z-10">
						...
						<div class="bg-[#08090D] border border-white/5 rounded-[20px] p-3.5 flex flex-col items-center gap-1.5 shadow-inner relative group overflow-hidden">
							...
							<div class="w-8 h-8 rounded-[10px] bg-white/5 text-[14px] font-black flex items-center justify-center text-white/60 mb-1 border border-white/10 shadow-sm">1</div>
							<span class="text-[11px] font-black text-white/80 uppercase tracking-widest">{t('channelFunnel.inputRaw')}</span>
							<span class="text-[10px] text-white/40 truncate w-full px-1 font-mono font-bold leading-snug">{funnel()?.input_title || t('channelFunnel.inputDrafts')}</span>
						</div>
						...
						<div class="bg-[#06b6d4]/10 border border-[#06b6d4]/30 rounded-[20px] p-3.5 flex flex-col items-center gap-1.5 shadow-[inset_0_0_15px_rgba(6,182,212,0.1)] relative overflow-hidden">
							...
							<div class="w-8 h-8 rounded-[10px] bg-[#06b6d4]/20 text-[#06b6d4] text-[14px] font-black flex items-center justify-center mb-1 border border-[#06b6d4]/40 shadow-sm relative z-10">2</div>
							<span class="text-[11px] font-black text-[#06b6d4] uppercase tracking-widest relative z-10">{t('channelFunnel.processAi')}</span>
							<span class="text-[10px] text-[#06b6d4]/70 font-bold w-full relative z-10">{t('channelFunnel.processReview')}</span>
						</div>
						...
						<div class="bg-[#10b981]/10 border border-[#10b981]/20 rounded-[20px] p-3.5 flex flex-col items-center gap-1.5 shadow-inner relative group overflow-hidden">
							...
							<div class="w-8 h-8 rounded-[10px] bg-[#10b981]/20 text-[#10b981] text-[14px] font-black flex items-center justify-center mb-1 border border-[#10b981]/30 shadow-sm relative z-10">3</div>
							<span class="text-[11px] font-black text-[#10b981] uppercase tracking-widest relative z-10">{t('channelFunnel.outputFinal')}</span>
							<span class="text-[10px] text-[#10b981]/60 truncate w-full px-1 font-mono font-bold leading-snug relative z-10">{channel()?.chat_title || t('channelFunnel.outputPublic')}</span>
						</div>
					</div>
				</div>
						...
						<Show
							...
							fallback={
								<div class="flex flex-col gap-5 relative z-10">
									<div class="bg-[#10b981]/10 border border-[#10b981]/20 rounded-[20px] p-5 flex flex-col gap-2 shadow-inner">
										<div class="flex items-center gap-2 mb-1">
											<div class="w-8 h-8 rounded-[10px] bg-[#10b981]/20 flex items-center justify-center border border-[#10b981]/30 shrink-0">
												<span class="material-symbols-outlined text-[16px] text-[#10b981]">check_circle</span>
											</div>
											<span class="text-[14px] font-black text-[#10b981] tracking-tight">{t('channelFunnel.activeDesc')}</span>
										</div>
									...
									<button
										onClick={handleDeleteFunnel} disabled={isSubmitting()}
									...
									>
										<Show when={isSubmitting()} fallback={<span class="material-symbols-outlined text-[20px]">delete_forever</span>}>
						...
						>
							<div class="flex flex-col gap-4 relative z-10">
								...
								<div class="flex flex-col gap-1.5">
									<label class="text-[11px] font-black text-white/50 uppercase tracking-widest px-1 flex items-center gap-1.5">
									...
									<div class="relative">
										<select
											...
											value={selectedInputChannel()} onChange={(e) => setSelectedInputChannel(e.currentTarget.value)}
										>
											<option value="" disabled class="text-white/40">{t('channelFunnel.noChannelSelected')}</option>
											<For each={userChannels()?.filter((c: any) => c.chat_id !== channel()?.chat_id)}>
												{(c) => <option value={c.chat_id}>{c.title}</option>}
											</For>
										</select>
										<span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-white/30 pointer-events-none text-[20px]">expand_more</span>
									</div>
								</div>

								<div class="flex flex-col gap-1.5 mt-2">
									<label class="text-[11px] font-black text-white/50 uppercase tracking-widest px-1 flex items-center gap-1.5">
										<span class="material-symbols-outlined text-[16px] text-white/40">alternate_email</span>
									...
									<input
										...
										value={inputIdentifier()} onInput={(e) => setInputIdentifier(e.currentTarget.value)}
								...
								<button
									onClick={handleCreateFunnel} disabled={!selectedInputChannel() || isSubmitting()}
								...
								>
									<Show when={isSubmitting()} fallback={<span class="material-symbols-outlined text-[20px]">play_arrow</span>}>
										<span class="material-symbols-outlined text-[20px] animate-spin">sync</span>
									</Show>
			...
			<ChannelHamburgerMenu isOpen={isMenuOpen()} onClose={() => setIsMenuOpen(false)} channelId={params.id} activeTab="funnel" />
		</div>
	);
};
