...
export const ChannelAdminsPage: Component = () => {
	...
	const allAdmins = createMemo(() => {
		...
		return list.map((a: any) => ({
			...
			perms: a.permissions || { post: true, edit: true, delete: true, pin: true, invite: true, videoChat: false, editInfo: false, manageTags: false, anonymous: false, promote: false, postStories: false, editStories: false, deleteStories: false },
		}));
	});
	...
	const openAdminModal = (admin: any = null) => {
		if (admin) {
			...
			if (!adminCopy.perms) {
				adminCopy.perms = { post: true, edit: true, delete: false, pin: false, invite: false, videoChat: false, editInfo: false, manageTags: false, anonymous: false, promote: false, postStories: false, editStories: false, deleteStories: false };
			}
			setEditingAdmin(adminCopy);
		} else {
			setEditingAdmin({
				id: Date.now().toString(), name: 'New Admin', role: 'Admin', customTitle: '',
				perms: { post: true, edit: true, delete: false, pin: false, invite: false, videoChat: false, editInfo: false, manageTags: false, anonymous: false, promote: false, postStories: false, editStories: false, deleteStories: false },
			});
		}
		setShowModal(true);
		haptic.impact('light');
	...
	return (
		<div class="min-h-screen bg-[#030303] pb-28 relative overflow-x-hidden text-white font-sans selection:bg-[#3390ec]/30" dir={isRtl() ? 'rtl' : 'ltr'}>
			...
			<div class="p-5 flex flex-col gap-5 max-w-md mx-auto relative z-10 w-full">
				...
				{/* ═══════ ACTIONS (Sync & Search) ═══════ */}
				<Motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} class="flex flex-col gap-3.5">
					<button
					...
					>
						<Show when={isSyncing()} fallback={<><span class="material-symbols-outlined text-[20px]">sync</span> {t('channelAdmins.syncFromTelegram')}</>}>
							<span class="material-symbols-outlined text-[20px] animate-spin">sync</span> {t('channelAdmins.syncing')}
						</Show>
					</button>

					<div class="relative z-10">
						<span class="absolute left-4 top-1/2 -translate-y-1/2 text-white/40 material-symbols-outlined text-[22px] pointer-events-none">search</span>
						<input
						...
						fallback={
							<div class="bg-[#12141C]/80 backdrop-blur-xl rounded-[24px] border border-white/5 py-12 flex flex-col items-center justify-center gap-3 shadow-sm border-dashed">
								<div class="w-16 h-16 rounded-[20px] bg-white/5 flex items-center justify-center border border-white/10 mb-1">
									<span class="material-symbols-outlined text-white/30 text-[36px]">search_off</span>
								</div>
						...
						<For each={groupedAdmins()}>
							{(group) => (
								<div class="flex flex-col gap-2">
									<h2 class="text-[11px] font-black text-white/40 uppercase tracking-widest px-2">{group.title}</h2>
											...
											{(item: any, i) => (
												...
												>
													<div class="flex items-center justify-between mb-3">
															...
															<div class={`w-12 h-12 rounded-[14px] flex items-center justify-center font-black text-[18px] relative shadow-inner shrink-0 ${item.role === 'Owner' ? 'bg-gradient-to-br from-amber-400/20 to-amber-400/5 text-amber-400 border border-amber-400/30' : item.role === 'Bot' ? 'bg-gradient-to-br from-cyan-400/20 to-cyan-400/5 text-cyan-400 border border-cyan-400/30' : 'bg-gradient-to-br from-[#3390ec]/20 to-[#3390ec]/5 text-[#3390ec] border border-[#3390ec]/30'}`}>
																...
																<Show when={item.role === 'Bot'}>
																	<div class="absolute -bottom-1 -right-1 w-5 h-5 bg-cyan-400 rounded-[6px] border border-[#08090D] flex items-center justify-center shadow-md">
																		<span class="material-symbols-outlined text-[12px] text-black font-black">smart_toy</span>
																	</div>
																</Show>
															</div>
															<div class="flex flex-col min-w-0">
																<div class="flex items-center gap-2 flex-wrap mb-0.5">
																	<span class="text-[15px] font-black text-white truncate max-w-[120px]">{item.name}</span>
																	<Show when={item.customTitle}>
																		<span class="bg-[#3390ec]/10 text-[#3390ec] text-[9px] px-2 py-0.5 rounded-[6px] border border-[#3390ec]/20 font-black uppercase tracking-wider">{item.customTitle}</span>
																	</Show>
																</div>
																<span class="text-[11px] text-white/50 font-bold truncate">
																	{item.username || (item.role === 'Owner' ? t('channelAdmins.owners') : item.role === 'Bot' ? t('channelAdmins.bots') : t('channelAdmins.currentAdmins'))}
																</span>
															</div>
														</div>
														...
														>
															<span class="material-symbols-outlined text-[18px] text-white/60">edit</span>
														</button>
													</div>

													<div class="flex flex-wrap gap-1.5 mt-1">
														<Show when={item.perms.post}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-[#10b981]/10 border-[#10b981]/20 text-[#10b981]">{t('channelAdmins.post')}</span></Show>
														<Show when={item.perms.edit}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-[#10b981]/10 border-[#10b981]/20 text-[#10b981]">{t('channelAdmins.edit')}</span></Show>
														<Show when={item.perms.delete}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-[#ff4a4a]/10 border-[#ff4a4a]/20 text-[#ff4a4a]">{t('channelAdmins.delete')}</span></Show>
														<Show when={item.perms.invite}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-[#3390ec]/10 border-[#3390ec]/20 text-[#3390ec]">{t('channelAdmins.invite')}</span></Show>
														<Show when={item.perms.promote}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-amber-400/10 border-amber-400/20 text-amber-400">{t('channelAdmins.promote')}</span></Show>
														<Show when={item.perms.postStories}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-cyan-400/10 border-cyan-400/20 text-cyan-400">{t('channelAdmins.stories')}</span></Show>
														<Show when={item.perms.anonymous}><span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-white/10 border-white/20 text-white">{t('channelAdmins.anonymous')}</span></Show>
														...
														<Show when={!item.perms.post && !item.perms.edit && !item.perms.delete && !item.perms.invite && !item.perms.promote && !item.perms.postStories && !item.perms.anonymous}>
															<span class="px-2 py-1 rounded-[6px] text-[9px] font-black uppercase tracking-widest border bg-transparent border-white/10 text-white/40">{t('channelAdmins.noSpecialPerms')}</span>
														</Show>
													</div>
												</Motion.div>
											)}
			...
			<Show when={showModal() && editingAdmin()}>
				<div class="fixed inset-0 bg-[#030303]/90 backdrop-blur-2xl z-[100] flex items-end sm:items-center justify-center p-4">
					<Motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ duration: 0.35, easing: [0.32, 0.72, 0, 1] }} class="bg-[#12141C] w-full max-w-md rounded-[32px] border border-white/10 flex flex-col max-h-[85vh] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.8)] relative">
						...
						<div class="p-5 border-b border-white/5 flex items-center justify-between sticky top-0 bg-[#12141C]/90 backdrop-blur-md z-20 shadow-sm">
							<h2 class="text-[18px] font-black text-white truncate pr-4">{editingAdmin()?.name}</h2>
							<button onClick={() => setShowModal(false)} class="w-9 h-9 rounded-[12px] bg-white/5 border border-white/10 flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 active:scale-95 transition-all shrink-0">
						...
						<div class="p-5 overflow-y-auto no-scrollbar flex flex-col gap-6">
							...
							{/* Custom Title Input */}
							<div class="flex flex-col gap-2">
								<label class="text-[11px] text-white/40 font-black uppercase tracking-widest ml-1">{t('channelAdmins.customTitle')}</label>
								<input
									type="text" value={editingAdmin().customTitle}
							...
							{/* Permissions Toggles */}
							<div class="flex flex-col gap-2">
								<label class="text-[11px] text-white/40 font-black uppercase tracking-widest ml-1 mb-1">{t('channelAdmins.permissions')}</label>
								<div class="bg-[#08090D] shadow-inner border border-white/5 rounded-[24px] p-2 flex flex-col">
									<For each={[
										...
										{(perm, index) => (
											<div class={`flex items-center justify-between p-3.5 ${index() !== 11 ? 'border-b border-white/5' : ''}`}>
						...
						<div class="p-5 border-t border-white/5 bg-[#12141C] sticky bottom-0 z-20">
							<button
								onClick={saveAdmin} disabled={isSaving()}
