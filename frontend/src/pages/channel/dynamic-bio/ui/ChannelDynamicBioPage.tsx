    ...
    <div class="min-h-screen bg-[#0f1014] pb-28 relative overflow-x-hidden text-white">
      ...
      <div class="px-5 pt-6 pb-4 bg-[#0f1014]/80 backdrop-blur-md sticky top-0 z-30 border-b border-[#1c1c1c] flex items-center justify-between gap-3">
        <div class="flex items-center gap-2 overflow-hidden flex-1">
          ...
          <div class="flex flex-col overflow-hidden">
            <h1 class="text-[18px] font-black text-white leading-tight truncate">{t('channelDynamicBio.title') || 'Dynamic Bio & Name'}</h1>
            <span class="text-[12px] text-on-surface-variant truncate">{t('channelDynamicBio.subtitle') || 'Live updating channel description'}</span>
          </div>
        </div>
      ...
      <div class="px-5 pt-6 flex flex-col gap-6">
          ...
          <Show when={enabled()}>
            ...
            <div class="flex flex-col gap-2">
              <label class="text-[13px] font-bold text-white flex justify-between">
                ...
                <span class={`text-[11px] font-bold ${bioTemplate().length > 255 ? 'text-[#ff3b30]' : 'text-[#32ade6]'}`}>
            ...
            <div class="mt-2 bg-[#2c2c2e] p-3 rounded-xl border border-[#3a3a3c] flex flex-col gap-3">
               <div class="flex items-center justify-between">
                  <div class="flex flex-col flex-1 min-w-0">
                    ...
                    <span class="text-[11px] text-[#8e8e93]">Add dynamic tags to the channel name</span>
                  </div>
            ...
            <SelectField 
               ...
               description={t('channelDynamicBio.updateIntervalDesc') || 'How often the channel info refreshes.'}
            />
          </Show>
        </Motion.div>
        ...
        <Show when={enabled()}>
           ...
           <Motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} class="bg-gradient-to-br from-[#1c1c1c] to-[#121212] rounded-3xl border border-[#32ade6]/30 p-5 flex flex-col gap-3 relative overflow-hidden">
             ...
             <div class="bg-[#0f1014] rounded-xl p-4 border border-[#2a2a2a] relative z-10 flex flex-col gap-2">
                <Show when={displayInName() && nameTemplate()}>
                   <div class="text-[16px] font-black text-white">
                      {nameTemplate().replace(/\$time/g, '14:30').replace(/\$countdown/g, '02:14:05')}
                   </div>
                </Show>
